...
const sections = computed(() =>
    [
        {
            type: 'course' as GroupType,
            label: 'Courses',
            groups: props.courses,
        },
        {
            type: 'cohort' as GroupType,
            label: 'Cohorts',
            groups: props.cohorts,
        },
        {
            type: 'program' as GroupType,
            label: 'Programs',
            groups: props.programs,
        },
...
const selectedKey = computed<string>({
    get: () =>
        form.attachable_type && form.attachable_id
            ? `${form.attachable_type}:${form.attachable_id}`
            : '',
    set: (value) => {
        if (!value) {
...
const selectedGroup = computed<GroupOption | null>(() => {
    ...
    return (
        section?.groups.find((g) => g.id === Number(form.attachable_id)) ?? null
    );
});
...
const uniqueStudentCount = computed(() => {
    ...
    sections.value.forEach((s) =>
        s.groups.forEach((g) => g.students.forEach((m) => ids.add(m.id))),
    );
    return ids.size;
});
...
function toggleAllStudents() {
    form.student_ids = allStudentsSelected.value
        ? []
        : availableStudents.value.map((m) => m.id);
}
...
<template>
    <div class="space-y-6">
        <div
            class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900/60"
        >
            <h2 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">
                Send notification
            </h2>
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                Send in-app and email notifications to course students or the
                interns you supervise.
            </p>
        </div>

        <form
            @submit.prevent="sendNotification"
            class="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900/60"
        >
            <div class="space-y-3">
                <label
                ...
                >
                    ...
                    >
                        <option
                            ...
                            :key="`${section.type}:${group.id}`"
                            :value="`${section.type}:${group.id}`"
                        >
            ...
            <div v-if="requiresStudents" class="space-y-2">
                ...
                <div
                    class="rounded-lg border border-gray-300 bg-white p-3 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
                >
                    <input
                    ...
                    <div class="max-h-56 space-y-2 overflow-y-auto">
                        ...
                        >
                            <input
                                type="checkbox"
                                :checked="form.student_ids.includes(member.id)"
                                class="mt-0.5"
                                @change="toggleStudent(member.id)"
                            />
                        ...
                        <p
                            v-else-if="searchableStudents.length === 0"
                            class="text-xs text-gray-500 dark:text-gray-400"
                        >
            ...
            >
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <span class="font-semibold">{{ recipientCount }}</span>
                </p>
                <button
                    type="submit"
