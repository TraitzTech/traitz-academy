...
const form = useForm({
    ...
    external_url: '',
    content: '',
    audience: 'all_program_interns' as
        | 'all_program_interns'
        | 'selected_students',
...
const selectedProgram = computed(
    () => props.programs.find((p) => p.id === Number(form.program_id)) ?? null,
);
...
watch(
    () => props.programs,
    (programs) => {
        if (programs.length && !form.program_id)
            form.program_id = programs[0].id;
    },
    { immediate: true },
);
...
const canSubmit = computed(() => {
    ...
    if (form.type === 'external_link' && !form.external_url.trim())
        return false;
...
function removeResource(resource: ResourceRow) {
    if (!confirm(`Remove "${resource.title}"? Interns will no longer see it.`))
        return;
    useForm({}).delete(`/tutor/resources/${resource.id}`, {
        preserveScroll: true,
    });
}
...
<template>
    <div class="space-y-6">
        ...
        >
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Share documents, videos, writing, or links with the interns
                    in your programs.
                </p>
            </div>
            <button
        ...
        >
            You don't supervise any programs yet, so there's nowhere to share a
            resource.
        </div>
        ...
        >
                ...
                <div>
                    ...
                    >
                        <option v-for="p in programs" :key="p.id" :value="p.id">
                            {{ p.title }} ({{ p.student_count }} interns)
                        </option>
                    </select>
                </div>
                <div>
            ...
            <div v-if="form.audience === 'selected_students'">
                ...
                >
                    ...
                    >
                        Select a program with interns first.
                    </p>
                </div>
            </div>
        ...
        >
            ...
            <div class="divide-y divide-gray-100 dark:divide-gray-700">
                <div
                    ...
                    :key="resource.id"
                    class="flex flex-wrap items-start justify-between gap-4 p-6"
                >
                    <div class="min-w-0 flex-1">
                        ...
                        <p
                            ...
                            class="mt-2 line-clamp-2 text-sm text-gray-700 dark:text-gray-300"
                        >
                    ...
                    >
                        <div>
                            {{
                                resource.audience === 'all_program_interns'
                                    ? 'Everyone in program'
                                    : `${resource.selected_students_count ?? 0} selected interns`
                            }}
                        </div>
                        <div class="text-gray-400 dark:text-gray-500">
                            Shared {{ formatDateTime(resource.created_at) }}
                        </div>
                        <button
                            type="button"
