...
const form = useForm({
    ...
    target_keys: (props.initialTargets ?? []).map((t) =>
        targetKey(t.type, t.id),
    ),
        ...
        props.students
            .filter((s) =>
                studentCohortFilter.value === ''
                    ? true
                    : s.cohort_id === studentCohortFilter.value,
            )
            .map((s) => s.program_id)
...
const filteredStudents = computed(() =>
    props.students.filter((s) => {
        ...
        if (
            ...
            s.program_id !== studentProgramFilter.value
        )
            return false;
        return true;
    }),
    ...
    <form class="space-y-4" @submit.prevent="submit">
        ...
        <div>
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                Leave blank to auto-generate a Google Meet link. Paste your own
                link to override.
            </p>
            <p v-if="form.errors.meeting_url" class="mt-1 text-xs text-red-500">
            ...
            <div v-if="form.access_type === 'course'" class="grid gap-3">
                <div v-for="typeGroup in targetGroups" :key="typeGroup[0]">
                    <label
                        ...
                        >{{ targetTypeLabels[typeGroup[0]] }}</label
                    >
                    <div
                        class="mt-1 max-h-32 space-y-1 overflow-y-auto rounded-lg border border-gray-200 p-2 dark:border-gray-600"
                    >
                        <label
                            v-for="option in typeGroup[1]"
                            :key="option.id"
                            class="flex items-center gap-2 text-sm"
                        >
                            <input
                                v-model="form.target_keys"
                                :value="targetKey(typeGroup[0], option.id)"
                                type="checkbox"
                            />
                            <span>{{ option.title || option.name }}</span>
                        </label>
                        <p
                            v-if="typeGroup[1].length === 0"
            ...
            <div v-else class="grid gap-2">
                ...
                <div v-if="cohorts.length > 0" class="flex flex-wrap gap-2">
                    ...
                    >
                        ...
                        <option
                            v-for="p in filterProgramOptions"
                            :key="p.id"
                            :value="p.id"
                        >
                            {{ p.title }}
                ...
                <div
                    class="max-h-44 space-y-1 overflow-y-auto rounded-lg border border-gray-200 p-2 dark:border-gray-600"
                >
                    <label
        ...
        <div class="flex justify-end">
            <button
                ...
                class="rounded-lg bg-[#381998] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
            >
