...
interface DiscussionRow {
  ...
  course?: { id: number; title: string | null; tutor_name?: string | null } | null
...
<template>
  <div>
    ...
    <div v-if="!hasRows" class="rounded-xl border border-dashed border-gray-300 p-10 text-center text-sm text-gray-500 dark:border-gray-600">
    ...
    <div v-else class="space-y-3">
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <p class="line-clamp-2 text-sm text-gray-700 dark:text-gray-200">{{ row.body }}</p>
            <p class="mt-2 text-xs text-gray-500">
              <span class="font-semibold">Author:</span> {{ row.author_name || 'Unknown' }}
              <span class="mx-1">·</span>
              <span class="font-semibold">Course:</span> {{ row.course?.title || '—' }}
              <template v-if="showTutorName && row.course?.tutor_name">
                <span class="mx-1">·</span>
                <span class="font-semibold">Tutor:</span> {{ row.course.tutor_name }}
              </template>
              <span class="mx-1">·</span>
              <span class="font-semibold">Lesson:</span> {{ row.lesson?.title || '—' }}
            </p>
            <p class="mt-1 text-xs text-gray-400">
              Posted {{ postedAt(row.created_at) }} · {{ row.replies_count }} repl{{ row.replies_count === 1 ? 'y' : 'ies' }}
            </p>
          </div>
          <span :class="['rounded-full px-2 py-0.5 text-xs font-semibold', rowStatus(row).classes]">{{ rowStatus(row).label }}</span>
        </div>
    ...
    <div v-if="discussions.links?.length > 3" class="mt-5 flex flex-wrap items-center gap-2">
      <Link
