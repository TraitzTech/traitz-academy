...
const allSelected = computed(() => {
  return props.stories.data.length > 0 && selectedIds.value.length === props.stories.data.length
})
...
const toggleSelectAll = () => {
  if (allSelected.value) {
    ...
    selectedIds.value = props.stories.data.map(s => s.id)
  }
}
...
const storyToDelete = ref<SuccessStory | null>(null)
...
const deleteStory = (story: SuccessStory) => {
...
const openBulkDeleteModal = () => {
  if (selectedIds.value.length === 0) {
    toast.error('Please select at least one story to delete.')
    return
  }
...
<template>
  <div>
    <Head title="Manage Success Stories" />

    <!-- Header -->
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h2 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Success Stories</h2>
        <p class="text-gray-600 dark:text-gray-400 mt-2">Manage student testimonials displayed on the homepage</p>
      </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Story</th>
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="story in stories.data" :key="story.id" :class="['hover:bg-gray-50 dark:hover:bg-gray-700', selectedIds.includes(story.id) ? 'bg-cyan-50 dark:bg-cyan-900/30' : '']">
              <td class="px-6 py-4 whitespace-nowrap">
                <input
                  type="checkbox"
                  :checked="selectedIds.includes(story.id)"
                  @change="toggleSelect(story.id)"
                  class="h-4 w-4 text-[#42b6c5] focus:ring-[#42b6c5] border-gray-300 dark:border-gray-600 rounded dark:bg-gray-700"
                />
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-12 w-12">
                    <img
                      ...
                      :src="story.image_url.startsWith('http') ? story.image_url : `/storage/${story.image_url}`"
                      :alt="story.name"
                  ...
                  <div class="ml-4">
                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ story.name }}</div>
                    <div class="text-sm text-gray-500 dark:text-gray-400">
                      {{ story.role }}<span v-if="story.company"> @ {{ story.company }}</span>
                    </div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="text-sm text-gray-600 dark:text-gray-400 max-w-xs truncate" :title="story.story">
                  "{{ story.story }}"
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <button
                  @click="toggleStatus(story)"
                  :class="[
                    'px-2 py-1 text-xs font-medium rounded-full transition-colors',
                    story.is_active ? 'bg-green-100 text-green-800 hover:bg-green-200 dark:bg-green-900/30 dark:text-green-400 dark:hover:bg-green-900/50' : 'bg-red-100 text-red-800 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50'
                  ]"
                >
            ...
            <tr v-if="stories.data.length === 0">
              <td colspan="6" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                No success stories found. Add your first story to display on the homepage.
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <!-- Pagination -->
      <div v-if="stories.links && stories.links.length > 3" class="bg-white dark:bg-gray-800 px-4 py-3 border-t border-gray-200 dark:border-gray-700 sm:px-6">
          ...
          <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
            <Link
              ...
              :class="[
                ...
                index === stories.links.length - 1 ? 'rounded-r-md' : ''
              ]"
              v-html="link.label"
            />
          </nav>
            ...
            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100">Delete Success Story</h3>
                  <div class="mt-2">
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                      Are you sure you want to delete the story from "<strong>{{ storyToDelete?.name }}</strong>"? This action cannot be undone.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div class="relative inline-block align-middle bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full">
            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100">Delete Multiple Stories</h3>
                  <div class="mt-2">
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                      Are you sure you want to delete <strong>{{ selectedIds.length }} story(ies)</strong>? This action cannot be undone.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
              <button
                ...
                class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm"
              >
                Delete {{ selectedIds.length }} Story(ies)
              </button>
              <button
