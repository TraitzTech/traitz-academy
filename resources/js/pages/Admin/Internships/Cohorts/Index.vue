...
const allProgramsSelected = computed(() => {
    const selected = new Set(
        form.programs.map((r) => r.program_id).filter(Boolean),
    );
    return (
...
function selectAllPrograms() {
    ...
    form.programs = props.programs.map((p) => ({
        program_id: p.id,
...
function toggleProgramsExpanded(cohortId: number) {
    if (expandedPrograms.value.has(cohortId))
        expandedPrograms.value.delete(cohortId);
...
<template>
    <div class="mx-auto max-w-6xl">
        ...
        <div class="mb-6 flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-0.5 text-sm text-gray-500">
                    A dated batch of interns spanning one or more programs, each
        ...
        <div
            v-if="showCreate"
            class="mb-6 overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"
        >
            <div
            ...
            <form class="space-y-4 p-6" @submit.prevent="submit">
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        ...
                        <input
                            ...
                            class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                        />
                        <p
                    ...
                    <div>
                        <label
                            class="mb-1 block text-sm font-semibold text-gray-700 dark:text-gray-200"
                            >Expected hours / day</label
                        >
                        <input
                            v-model="form.expected_hours_per_day"
                            type="number"
                            min="0"
                            max="24"
                            step="0.5"
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        >
                            <input
                                ...
                                @change="toggleDay(form.working_days, d.value)"
                            />
                            {{ d.label }}
                        </label>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">
                        Interns in this cohort must submit a logbook entry on
                        these days. Leave the app default (Mon–Fri) unless this
                        cohort works a different schedule.
                    </p>
                </div>
                ...
                >
                    ...
                    <p class="mb-3 text-xs text-gray-500">
                        When set, this opens/closes applications for
                        <strong>every program in this cohort</strong> — enter
                        the intake dates once here. Leave blank to keep each
                        program's own window (or rolling).
                    </p>
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            <label
                ...
                >
                    ...
                    <p class="mb-3 text-xs text-gray-500">
                        Each program gets its own supervisor. Accepted
                        applicants for a program join this cohort under that
                        supervisor.
                    </p>
                    <div
                        v-for="(row, i) in form.programs"
                        :key="i"
                        class="mb-2 flex items-center gap-2"
                    >
                        <select
                            v-model="row.program_id"
                            class="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                        >
                ...
                >
                    ...
                    <span>
                        ...
                        <span class="block text-xs text-gray-500"
                            >Newly-accepted applicants are auto-placed in this
                            cohort (under their program's supervisor). Only one
                            intake cohort at a time.</span
                        >
                    </span>
                </label>
                ...
                <div class="flex items-center justify-end gap-3">
                    <button
                        type="button"
                        class="rounded-xl px-4 py-2 text-sm font-medium text-gray-600 hover:text-[#381998]"
                    ...
                    <button
                        ...
                        class="rounded-xl bg-[#381998] px-6 py-2.5 text-sm font-semibold text-white hover:bg-[#000928] disabled:opacity-60"
                    >
        ...
        >
            <div
                v-if="cohorts.data.length === 0"
                class="p-10 text-center text-sm text-gray-500"
            >
                No cohorts yet. Create one and add its programs to start
                receiving accepted interns.
            </div>
            <table v-else class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                    ...
                    >
                        <td
                            class="px-5 py-3 font-semibold text-[#000928] dark:text-white"
                        >
                            {{ c.name }}
                            <span
                                v-if="c.is_intake"
                        ...
                        <td class="max-w-xs px-5 py-3">
                            <span
                                v-if="c.programs.length === 0"
                                class="text-gray-400"
                                >—</span
                            >
                            <div v-else class="flex flex-wrap gap-1">
                                <span
                                    v-for="p in expandedPrograms.has(c.id)
                                        ? c.programs
                                        : c.programs.slice(
                                              0,
                                              PROGRAM_BADGE_LIMIT,
                                ...
                                <button
                                    v-if="
                                        c.programs.length > PROGRAM_BADGE_LIMIT
                                    "
                                    type="button"
                                ...
                                >
                                    {{
                                        expandedPrograms.has(c.id)
                                            ? 'Show less'
                                            : `+${c.programs.length - PROGRAM_BADGE_LIMIT} more`
                                    }}
                                </button>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-3 text-right">
                            <div class="flex items-center justify-end gap-3">
                                <Link
                                    :href="`/admin/internships/cohorts/${c.id}`"
            ...
            <button
                ...
                :class="
                    link.active
                        ...
                        : 'text-gray-600 hover:bg-gray-100 disabled:opacity-40 dark:text-gray-300'
                "
                @click="
                    link.url &&
