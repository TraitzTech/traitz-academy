...
import { ArrowLeft, CalendarDays, CheckCheck, ChevronDown, FileCheck2, Globe, LayoutList, Plus, Search, Settings2, Trash2, UserMinus, UserPlus, Users, X } from 'lucide-vue-next'
...
const field = 'w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-[#42b6c5] focus:outline-none focus:ring-2 focus:ring-[#42b6c5]/20'
const fieldSm = 'rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-800 dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-[#42b6c5] focus:outline-none focus:ring-2 focus:ring-[#42b6c5]/20'
const label = 'mb-1 block text-sm font-semibold text-gray-700 dark:text-gray-200'
const primaryBtn = 'inline-flex items-center gap-2 rounded-xl bg-[#42b6c5] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#35919e] disabled:cursor-not-allowed disabled:opacity-50'
const textBtn = 'inline-flex items-center gap-1 text-xs font-semibold text-[#381998] hover:underline'
...
const allProgramsSelected = computed(() => {
  const selected = new Set(settings.programs.map((r) => r.program_id).filter(Boolean))
  return props.programs.length > 0 && props.programs.every((p) => selected.has(p.id))
})
function selectAllPrograms() {
  const existing = new Map(settings.programs.filter((r) => r.program_id).map((r) => [r.program_id, r.supervisor_id]))
  settings.programs = props.programs.map((p) => ({ program_id: p.id, supervisor_id: existing.get(p.id) ?? '' }))
}
...
const availProgramCounts = computed(() => {
  ...
  return [...map.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([program, count]) => ({ program, count }))
})

const availableFiltered = computed(() => props.available.filter((a) => {
  if (availProgram.value && a.program !== availProgram.value) return false
...
const allVisibleSelected = computed(() => {
  ...
  return visible.length > 0 && visible.every((a) => selectedInterns.value.includes(a.id))
})
...
function addSelectedInterns() {
  ...
  router.post(`/admin/internships/cohorts/${props.cohort.id}/interns`, { internship_ids: selectedInterns.value }, {
    preserveScroll: true,
    onSuccess: () => { selectedInterns.value = [] },
    onError: (e) => toast.error((Object.values(e)[0] as string) || 'Could not add intern(s).'),
  })
}
...
watch(manualQuery, (q) => {
  if (suppressNextQueryWatch) { suppressNextQueryWatch = false; return }
  manualSelectedUser.value = null
  clearTimeout(searchTimer)
  if (q.trim().length < 2) { manualResults.value = []; return }
  searchTimer = setTimeout(async () => {
    manualSearching.value = true
    try {
      const res = await fetch(`/admin/internships/users/search?q=${encodeURIComponent(q)}`, { headers: { Accept: 'application/json' } })
...
function submitManualIntern() {
  manualForm
    .transform((data) => ({
      ...
      user_id: manualMode.value === 'existing' ? (manualSelectedUser.value?.id ?? '') : '',
    }))
...
const rosterFiltered = computed(() => {
  ...
  return props.interns.filter((i) => {
    if (rosterProgram.value && i.program !== rosterProgram.value) return false
    if (q && !(`${i.name ?? ''} ${i.email ?? ''}`.toLowerCase().includes(q))) return false
    return true
  })
})

const rosterFilterActive = computed(() => Boolean(rosterProgram.value || rosterSearch.value))
...
type RosterRow = { type: 'header'; program: string; count: number } | { type: 'intern'; intern: Intern }
const rosterRows = computed<RosterRow[]>(() => {
  if (!groupByProgram.value) {
    return rosterFiltered.value.map((intern) => ({ type: 'intern', intern }))
  }
  ...
  for (const [program, list] of [...map.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
...
<template>
  <div class="space-y-6">
      ...
      <div class="flex flex-wrap items-start justify-between gap-4 p-6">
        <div>
          <div class="flex flex-wrap items-center gap-2.5">
            ...
            <span :class="['rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize', statusStyles[cohort.status] ?? statusStyles.completed]">{{ cohort.status }}</span>
            <span v-if="cohort.is_intake" class="rounded-full bg-[#381998]/10 px-2.5 py-0.5 text-xs font-semibold text-[#381998] dark:bg-[#381998]/30 dark:text-[#b9a6f0]">Intake cohort</span>
          </div>
          <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-500 dark:text-gray-400">
            <span class="inline-flex items-center gap-1.5"><CalendarDays class="h-4 w-4" /> {{ cohort.start_date }} → {{ cohort.end_date }}</span>
            <span class="inline-flex items-center gap-1.5"><Users class="h-4 w-4" /> {{ interns.length }} intern{{ interns.length === 1 ? '' : 's' }}</span>
            <span v-if="cohort.timezone" class="inline-flex items-center gap-1.5"><Globe class="h-4 w-4" /> {{ cohort.timezone }}</span>
          </div>
          <div v-if="cohort.programs.length > 0" class="mt-3 flex flex-wrap gap-1.5">
            <span v-for="p in cohort.programs" :key="p.id" class="rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-700 dark:text-gray-300">
              {{ p.title }}
            </span>
          </div>
        </div>
        <div class="flex shrink-0 gap-2">
          ...
          <button type="button" :class="primaryBtn" @click="addOpen = !addOpen">
    ...
    <!-- Settings (on demand) -->
    <div v-if="settingsOpen" class="rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-700">
        <h2 class="text-sm font-bold text-[#000928] dark:text-white">Cohort settings</h2>
        <button type="button" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" @click="settingsOpen = false"><X class="h-4 w-4" /></button>
      </div>
      <div class="space-y-4 p-6">
        <div class="grid gap-4 sm:grid-cols-2">
          <div>
            <label :class="label">Name</label>
            <input v-model="settings.name" type="text" :class="field" />
          </div>
          <div>
          ...
          <div>
            ...
            <input v-model="settings.start_date" type="date" :class="field" />
          </div>
          <div>
        ...
        <div class="rounded-xl border border-dashed border-gray-300 p-4 dark:border-gray-600">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input v-model="settings.intake_opens_at" type="date" :class="field" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-300">Applications close</label>
              <input v-model="settings.intake_closes_at" type="date" :class="field" />
            </div>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 p-4 dark:border-gray-600">
          ...
          <div v-for="(row, i) in settings.programs" :key="i" class="mb-2 flex items-center gap-2">
            <select v-model="row.program_id" :class="[fieldSm, 'flex-1 px-3 py-2']">
              <option value="">Select program…</option>
              <option v-for="p in programs" :key="p.id" :value="p.id">{{ p.title }}</option>
            </select>
            <select v-model="row.supervisor_id" :class="[fieldSm, 'flex-1 px-3 py-2']">
          ...
          <div class="mt-1 flex flex-wrap items-center gap-4">
            <button type="button" :class="textBtn" @click="addProgramRow">
              <Plus class="h-3.5 w-3.5" /> Add program
            </button>
            <button v-if="!allProgramsSelected" type="button" :class="textBtn" @click="selectAllPrograms">
        ...
        <div class="flex justify-end">
          <button :disabled="settings.processing" class="rounded-xl bg-[#381998] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#000928] disabled:opacity-60" @click="saveSettings">
    ...
    <div v-if="addOpen" class="rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div class="flex items-center justify-between border-b border-gray-100 px-6 py-3 dark:border-gray-700">
        <div class="flex gap-1">
          <button
            type="button"
            :class="['inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold transition-colors', addTab === 'applications' ? 'bg-[#381998] text-white' : 'text-gray-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700/50']"
            @click="addTab = 'applications'"
          >
            <FileCheck2 class="h-4 w-4" /> From applications
            <span v-if="available.length > 0" :class="['rounded-full px-1.5 text-[11px]', addTab === 'applications' ? 'bg-white/20' : 'bg-gray-100 dark:bg-gray-700']">{{ available.length }}</span>
          </button>
          <button
            type="button"
            :class="['inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold transition-colors', addTab === 'manual' ? 'bg-[#381998] text-white' : 'text-gray-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700/50']"
            @click="addTab = 'manual'"
          >
            <UserPlus class="h-4 w-4" /> Manually
          </button>
        </div>
        <button type="button" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" @click="addOpen = false"><X class="h-4 w-4" /></button>
      </div>

      <!-- Tab: accepted applicants -->
      <div v-if="addTab === 'applications'" class="space-y-3 p-6">
        <p class="text-xs text-gray-500">Accepted applicants not yet in any cohort, for the programs this cohort runs. Filter by acceptance date to isolate a batch — e.g. everyone accepted before cohorts existed.</p>

        <div v-if="available.length === 0" class="rounded-xl border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500 dark:border-gray-600">
          No unassigned interns right now. Interns appear here once their application is accepted (or auto-join if this is the intake cohort).
        </div>
        <template v-else>
          <div class="space-y-3 rounded-xl bg-gray-50 p-3 dark:bg-gray-900/40">
            <!-- Program picker: the primary way to add a whole program's batch at once -->
            <div v-if="availProgramCounts.length > 1" class="flex flex-wrap items-center gap-1.5">
              <button
                type="button"
                :class="['rounded-full px-3 py-1 text-xs font-semibold transition-colors', availProgram === '' ? 'bg-[#381998] text-white' : 'bg-white text-gray-600 ring-1 ring-gray-200 hover:ring-[#381998] dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-600']"
                @click="availProgram = ''"
              >
                All <span class="opacity-70">· {{ available.length }}</span>
              </button>
              <button
                v-for="pc in availProgramCounts"
                :key="pc.program"
                type="button"
                :class="['rounded-full px-3 py-1 text-xs font-semibold transition-colors', availProgram === pc.program ? 'bg-[#381998] text-white' : 'bg-white text-gray-600 ring-1 ring-gray-200 hover:ring-[#381998] dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-600']"
                @click="availProgram = pc.program"
              >
                {{ pc.program }} <span class="opacity-70">· {{ pc.count }}</span>
              </button>
            </div>
            ...
            <div class="flex flex-wrap items-end gap-3">
              ...
              <div>
                <label class="mb-1 block text-xs font-semibold text-gray-500">Accepted to</label>
                <input v-model="acceptedTo" type="date" :class="fieldSm" />
              </div>
              <button v-if="availFilterActive" type="button" class="pb-2 text-xs font-semibold text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" @click="clearAvailFilters">
                Clear filters
              </button>
              <div class="ml-auto flex items-center gap-3 pb-1">
                <span class="text-xs text-gray-400">{{ availableFiltered.length }} of {{ available.length }} shown</span>
                <button v-if="availableFiltered.length > 0" type="button" :class="textBtn" @click="toggleSelectAllInterns">
                  {{ allVisibleSelected ? 'Clear all' : 'Select all' }}{{ availFilterActive ? ' shown' : '' }}
                </button>
              </div>
            </div>
          </div>

          <div v-if="availableFiltered.length === 0" class="rounded-xl border border-gray-200 p-4 text-center text-sm text-gray-500 dark:border-gray-600">
            No interns match these filters.
          </div>
          <div v-else class="max-h-72 space-y-1 overflow-y-auto rounded-xl border border-gray-200 p-2 dark:border-gray-600">
            <label
            ...
            >
              <input type="checkbox" :checked="selectedInterns.includes(a.id)" class="accent-[#381998]" @change="toggleSelectedIntern(a.id)" />
              <span class="min-w-0">
                <span class="block truncate font-medium text-gray-800 dark:text-gray-100">{{ a.name }}</span>
                <span class="block truncate text-xs text-gray-400">{{ a.program }} · {{ a.email }}</span>
              </span>
              <span v-if="a.accepted_at" class="ml-auto shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-500 dark:bg-gray-700 dark:text-gray-300">
          ...
          <div class="flex justify-end">
            <button :disabled="selectedInterns.length === 0" :class="primaryBtn" @click="addSelectedInterns">
              <UserPlus class="h-4 w-4" /> Add {{ selectedInterns.length || '' }} intern{{ selectedInterns.length === 1 ? '' : 's' }}
            </button>
          </div>
        </template>
      </div>
      ...
      <div v-else class="space-y-4 p-6">
        <p class="text-xs text-gray-500">For someone who never submitted an application. Pick an existing account, or create a new one — login credentials are emailed automatically.</p>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <select v-model="manualForm.program_id" :class="field">
              <option value="">Select program…</option>
              <option v-for="p in cohort.programs" :key="p.id" :value="p.id">{{ p.title }}</option>
            </select>
          </div>
          <div>
            <label :class="label">Person</label>
            <div class="flex gap-1 rounded-xl border border-gray-200 p-1 dark:border-gray-600">
              <button
                type="button"
                :class="['flex-1 rounded-lg py-1.5 text-sm font-semibold transition-colors', manualMode === 'existing' ? 'bg-[#381998] text-white' : 'text-gray-600 dark:text-gray-300']"
                @click="manualMode = 'existing'"
              >Existing user</button>
              <button
                type="button"
                :class="['flex-1 rounded-lg py-1.5 text-sm font-semibold transition-colors', manualMode === 'new' ? 'bg-[#381998] text-white' : 'text-gray-600 dark:text-gray-300']"
        ...
        <div v-if="manualMode === 'existing'">
          ...
          <div class="relative">
            ...
            <input v-model="manualQuery" type="text" placeholder="Start typing a name or email…" :class="[field, 'pl-9']" />
          </div>
          <div v-if="manualResults.length > 0" class="mt-1 max-h-48 overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-600">
            <button
            ...
            >
              <span class="font-medium text-gray-800 dark:text-gray-100">{{ u.name }}</span>
              <span class="text-xs text-gray-400">{{ u.email }}</span>
            </button>
          </div>
          <p v-if="manualSearching" class="mt-1 text-xs text-gray-400">Searching…</p>
        </div>

        <div v-else class="grid gap-4 sm:grid-cols-2">
          <div>
            <label :class="label">Full name</label>
            <input v-model="manualForm.name" type="text" :class="field" />
          </div>
          <div>
            <label :class="label">Email</label>
            <input v-model="manualForm.email" type="email" :class="field" />
          </div>
          <div class="sm:col-span-2">
            <label :class="label">Phone <span class="font-normal text-gray-400">(optional)</span></label>
            <input v-model="manualForm.phone" type="text" :class="field" />
          </div>
        </div>

        <div class="flex justify-end">
          <button :disabled="!canSubmitManual" :class="primaryBtn" @click="submitManualIntern">
            <UserPlus class="h-4 w-4" /> {{ manualForm.processing ? 'Adding…' : 'Add intern' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-6 py-4 dark:border-gray-700">
        <h2 class="text-sm font-bold text-[#000928] dark:text-white">Interns ({{ interns.length }})</h2>
        <div v-if="interns.length > 0" class="flex flex-wrap items-center gap-2">
          <div class="relative">
            <Search class="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-gray-400" />
            <input v-model="rosterSearch" type="text" placeholder="Search name or email…" :class="[fieldSm, 'w-48 pl-8']" />
          </div>
          <select v-model="rosterProgram" :class="fieldSm">
            <option value="">All programs</option>
            <option v-for="p in cohort.programs" :key="p.id" :value="p.title">{{ p.title }}</option>
          </select>
          <button
            type="button"
            :class="['inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold transition-colors', groupByProgram ? 'border-[#381998] bg-[#381998]/10 text-[#381998] dark:bg-[#381998]/30 dark:text-[#b9a6f0]' : 'border-gray-200 text-gray-600 hover:border-[#381998] hover:text-[#381998] dark:border-gray-600 dark:text-gray-300']"
      ...
      <div v-if="interns.length === 0" class="p-10 text-center">
        ...
        <p class="mt-2 text-sm text-gray-500">No interns in this cohort yet.</p>
        <button type="button" class="mt-3 text-sm font-semibold text-[#381998] hover:underline" @click="addOpen = true">Add your first interns</button>
      </div>
      <div v-else-if="rosterFiltered.length === 0" class="p-10 text-center text-sm text-gray-500">
        No interns match your filters.
        <button v-if="rosterFilterActive" type="button" class="ml-1 font-semibold text-[#381998] hover:underline" @click="rosterProgram = ''; rosterSearch = ''">Clear</button>
      </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
            <template v-for="(row, idx) in rosterRows" :key="row.type === 'intern' ? `i-${row.intern.id}` : `h-${row.program}-${idx}`">
              ...
              <tr v-else>
                <td class="px-6 py-3">
                  <p class="font-semibold text-[#000928] dark:text-white">{{ row.intern.name }}</p>
                  <p class="text-xs text-gray-400">{{ row.intern.email }}</p>
                </td>
                <td class="px-6 py-3 text-gray-600 dark:text-gray-300">{{ row.intern.program }}</td>
                <td class="px-6 py-3">
                  <select
                    :value="row.intern.effective_supervisor_id ?? ''"
                    :class="[fieldSm, 'text-xs']"
                    @change="setInternSupervisor(row.intern.id, ($event.target as HTMLSelectElement).value)"
                  >
