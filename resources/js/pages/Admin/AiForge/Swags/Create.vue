...
const addVariationType = () => {
    variationTypes.value.push({ type: '', options: [{ name: '', imageFile: null, imagePreview: null }] })
}
...
const addOption = (typeIdx: number) => {
    variationTypes.value[typeIdx].options.push({ name: '', imageFile: null, imagePreview: null })
}
...
const submit = () => {
    form.transform((data) => {
        ...
        forceFormData: true,
        onSuccess: () => toast.success('Swag item created successfully!'),
        onError: () => toast.error('Failed to create swag. Please check the form for errors.'),
    })
}
</script>
...
<template>
    <div>
        ...
        <div class="mb-8">
            ...
            <h2 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Add New Swag</h2>
            <p class="text-gray-600 dark:text-gray-400 mt-2">Add merchandise for {{ event?.title ?? 'AI Forge' }}</p>
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <div v-if="!variationTypes.length" class="text-center py-8 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                    ...
                    <div class="flex flex-wrap gap-2 justify-center mt-3">
                        <button v-for="preset in variationPresets" :key="preset" type="button" @click="variationTypes.push({ type: preset, options: [{ name: '', imageFile: null, imagePreview: null }] })" class="px-3 py-1.5 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                            + {{ preset }}
                        </button>
                    </div>
                </div>
