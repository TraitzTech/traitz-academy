...
const approxPerInstallment = computed(() => {
    ...
    const sale =
        pricingForm.sale_price === '' || pricingForm.sale_price === null
            ? null
            : parseFloat(String(pricingForm.sale_price));
    const effective =
        sale != null && sale > 0 && sale < priceNum.value
            ? sale
            : priceNum.value;
...
function savePricing() {
    pricingForm.put(`/admin/courses/${props.course.id}/pricing`, {
        preserveScroll: true,
    });
}
</script>
        ...
        <div class="mx-auto max-w-3xl">
            <div class="rounded-xl bg-white p-6 shadow dark:bg-gray-800">
                <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">
                    Same model as programs: set the course price and how many
                    installments are allowed. Each installment is the total fee
                    divided by this number (before online surcharge at
                    checkout).
                </p>
                <form class="space-y-4" @submit.prevent="savePricing">
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            />
                            <p
                                v-if="pricingForm.errors.sale_price"
                                class="mt-1 text-xs text-red-600"
                            >
                                {{ pricingForm.errors.sale_price }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            max="12"
                            :disabled="priceNum <= 0"
                            class="w-full max-w-xs rounded-lg border border-gray-300 px-3 py-2 disabled:opacity-60 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
                        />
                        <p
                            class="mt-1 text-xs text-gray-500 dark:text-gray-400"
                        >
                            Set to 1 for one-time payment. Free courses always
                            use 1.
                        </p>
                        <p
                            v-if="
                                priceNum > 0 &&
                                (pricingForm.max_installments ?? 1) > 1
                            "
                            class="mt-2 text-sm text-gray-600 dark:text-gray-300"
                        >
                            ≈ {{ approxPerInstallment.toLocaleString() }} XAF
                            per installment (of
                            {{ pricingForm.max_installments }}).
                        </p>
                        <p
                            v-if="pricingForm.errors.max_installments"
                            class="mt-1 text-xs text-red-600"
                        >
                            {{ pricingForm.errors.max_installments }}
                        </p>
                    </div>

                    <label class="flex cursor-pointer items-center gap-2">
                        <input
                            v-model="pricingForm.is_featured"
                            type="checkbox"
                            class="rounded border-gray-300"
                        />
                        <span class="text-sm text-gray-700 dark:text-gray-300"
                            >Featured on catalogue</span
                        >
                    </label>
