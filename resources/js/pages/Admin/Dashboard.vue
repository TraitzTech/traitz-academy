...
function confirmApprove() {
    router.post(
        `/admin/courses/${courseTarget.value.id}/approve`,
        {},
        {
            preserveScroll: true,
            onFinish: () => {
...
function confirmReject() {
    router.post(
        `/admin/courses/${courseTarget.value.id}/reject`,
        {},
        {
            preserveScroll: true,
            onFinish: () => {
...
const enrollmentBadgeClass = {
    ...
    completed:
        'bg-[#42b6c5]/15 text-[#2a8a96] dark:bg-[#42b6c5]/20 dark:text-[#7ee8f9]',
    suspended:
        'bg-amber-100 text-amber-900 dark:bg-amber-900/30 dark:text-amber-300',
};
function enrollmentStatusClass(status) {
    return (
        enrollmentBadgeClass[status] ||
...
const allStatCards = computed(() => [
    ...
    },
    {
        ...
        icon: Clock,
        tint: 'amber',
        value: props.stats.pending_courses,
    },
    {
        key: 'total_collected',
        label: props.stats.collected_label || 'Collected',
        icon: DollarSign,
        tint: 'accent',
        value: formatMoney(props.stats.total_collected || 0),
    },
]);
const tintClass = {
    accent: 'bg-[#42b6c5]/10 text-[#2a8a96] dark:bg-[#42b6c5]/15 dark:text-[#7ee8f9]',
    secondary:
        'bg-[#381998]/10 text-[#381998] dark:bg-[#381998]/20 dark:text-purple-300',
    primary:
        'bg-[#000928]/10 text-[#000928] dark:bg-white/10 dark:text-gray-200',
...
<template>
    <div class="space-y-6">
        ...
        <div>
            <h1
                class="text-2xl font-bold text-[#000928] lg:text-3xl dark:text-white"
            >
                Dashboard
            </h1>
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            ...
            >
                ...
                <p
                    class="mt-4 text-2xl font-bold text-[#000928] tabular-nums dark:text-white"
                >
                    {{ card.value }}
                </p>
                <p
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            >
                ...
                <p
                    class="mt-1 text-3xl font-bold text-[#000928] dark:text-white"
                >
                    {{ stats.lms_distinct_learners ?? 0 }}
                </p>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Unique users with at least one non-revoked enrollment
                </p>
            </div>
            <div
                class="flex items-start justify-between gap-3 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800"
            >
                <div>
                    <p
                        class="text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400"
                    ...
                    <p
                        class="mt-1 text-3xl font-bold text-[#000928] dark:text-white"
                    >
                        {{ stats.lms_total_enrollments ?? 0 }}
                    </p>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Active enrollment rows (excludes revoked)
                    </p>
                </div>
                <a
                ...
                <a
                    v-for="report in [
                        ...
                        },
                        {
                            ...
                            label: 'Per-course report',
                        },
                        {
                            href: '/admin/lms/user-reports',
                            label: 'Per-user report',
                        },
                        {
            ...
            <div class="divide-y divide-gray-50 dark:divide-gray-700/50">
                ...
                >
                    <div class="min-w-0">
                        ...
                        >
                            {{ row.course_title }} · {{ row.tutor_name }}
                        </p>
                    </div>
                    <div class="flex shrink-0 items-center gap-3">
                        <span
                            :class="[
                                ...
                                enrollmentStatusClass(row.access_status),
                            ]"
                        >
                            {{ row.access_status }}
                        </span>
        ...
        >
            ...
            <div
                ...
                class="divide-y divide-gray-50 lg:hidden dark:divide-gray-700/50"
            >
                <div
                ...
                >
                    ...
                    <div class="flex items-center justify-between text-sm">
                        <span
                            class="max-w-[60%] truncate text-gray-600 dark:text-gray-400"
                            >{{ app.program.title }}</span
                        >
                        <span
            ...
            >
                <table class="w-full text-sm">
                    <thead
                        class="border-b border-gray-100 text-left text-xs tracking-wide text-gray-500 uppercase dark:border-gray-700 dark:text-gray-400"
                    >
                        <tr>
                    ...
                    >
                        ...
                        >
                            <td
                                class="px-5 py-3 font-semibold text-gray-900 dark:text-gray-100"
                            >
                                {{ app.first_name }} {{ app.last_name }}
                            </td>
                            <td
        ...
        >
            ...
            >
                <div class="flex items-center gap-2.5">
                    ...
                    <span
                        ...
                        class="inline-flex items-center justify-center rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-bold text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
                    >
                ...
                <div class="flex items-center gap-2">
                    <a
                        href="/admin/courses"
                        class="rounded-xl border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-600 transition-colors hover:border-gray-400 dark:border-gray-600 dark:text-gray-300"
                    >
                        Manage Courses
                    </a>
                    <a
                        href="/admin/courses?status=pending_review"
                        class="rounded-xl bg-[#42b6c5] px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-[#35919e]"
                    >
                        Pending Queue
                    </a>
                </div>
            ...
            >
                ...
                />
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    No courses awaiting review — you're all caught up!
                </p>
            ...
            <div v-else class="divide-y divide-gray-50 dark:divide-gray-700/50">
                <div
                    ...
                    :key="course.id"
                    class="flex flex-col gap-3 px-5 py-4 transition-colors hover:bg-gray-50/60 sm:flex-row sm:items-center sm:justify-between dark:hover:bg-gray-700/30"
                >
                    <!-- Course info -->
                    <div class="flex min-w-0 items-start gap-4">
                        <div
                            class="h-14 w-20 shrink-0 overflow-hidden rounded-xl bg-gradient-to-br from-[#381998] to-[#42b6c5]"
                        >
                            <img
                        ...
                        <div class="min-w-0">
                            ...
                            >
                                <span v-if="course.instructor"
                                    >by {{ course.instructor.name }}</span
                                >
                                <span
                                    v-if="course.category"
                                    class="rounded-full bg-[#381998]/10 px-2 py-0.5 font-medium text-[#381998] dark:bg-[#381998]/20 dark:text-purple-300"
                                >
