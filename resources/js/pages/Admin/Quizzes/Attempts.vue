...
<template>
  <AppLayout>
    ...
    <div class="mb-6 flex items-center justify-between">
      ...
      <Link :href="`/admin/courses/${quiz.course.id}`" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold">Back to Course</Link>
    </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="a in attempts.data" :key="a.id" class="border-t border-gray-100 dark:border-gray-700">
            ...
            <td class="px-4 py-3 text-right"><Link :href="`/admin/quizzes/${quiz.id}/attempts/${a.id}`" class="font-semibold text-[#381998]">View</Link></td>
          </tr>
        </tbody>
      </table>
    </div>
