...
function saveSettings() {
    settingsForm.put(
        `/tutor/courses/${props.course.id}/lessons/${props.lesson.id}/quiz`,
        { preserveScroll: true },
    );
}
...
function loadQuestionForEdit(q: QuizQuestion) {
    ...
    if (q.type === 'multiple_select') {
        questionForm.correct_answer = (q.correct_answer ?? []).map((v) =>
            Number(v),
        );
    } else if (q.type === 'short_answer') {
        questionForm.correct_answer = String(
            (q.correct_answer ?? [''])[0] ?? '',
        );
    } else if (q.type === 'true_false') {
        questionForm.correct_answer = String(
            (q.correct_answer ?? ['false'])[0] ?? 'false',
        );
    } else {
...
function submitQuestion() {
    ...
    if (editingQuestionId.value) {
        questionForm.put(
            `/tutor/quizzes/${props.quiz.id}/questions/${editingQuestionId.value}`,
            {
                data: payload,
                preserveScroll: true,
...
function deleteQuestion(id: number) {
    if (!props.quiz) return;
    router.delete(`/tutor/quizzes/${props.quiz.id}/questions/${id}`, {
        preserveScroll: true,
    });
}
...
function moveQuestion(from: number, to: number) {
    ...
    router.post(
        `/tutor/quizzes/${props.quiz.id}/questions/reorder`,
        { order: ids },
        { preserveScroll: true },
    );
}
...
function toggleMultiSelectOption(index: number, checked: boolean) {
    const current = Array.isArray(questionForm.correct_answer)
        ...
        <div class="mx-auto max-w-6xl">
            <div class="mb-6 flex items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2">
                    <Link
                        ...
                        class="rounded-lg border border-[#42b6c5] px-4 py-2 text-sm font-semibold text-[#42b6c5] hover:bg-[#42b6c5]/10"
                    >
                        View Attempts
                    </Link>
                    <Link
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                <div class="space-y-6 lg:col-span-2">
                    <div
                        class="rounded-xl bg-white p-6 shadow dark:bg-gray-800"
                    >
                        <h2
                        ...
                        >
                            ...
                            <div>
                                ...
                                <input
                                    v-model.number="
                                        settingsForm.pass_mark_percentage
                                    "
                                    type="number"
                                    min="0"
                                    max="100"
                            ...
                            <label
                                ...
                                ><input
                                    v-model="settingsForm.is_required"
                                    type="checkbox"
                                    class="rounded"
                                />
                                Required to complete lesson</label
                            >
                            <label
                                class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200"
                                ><input
                                    v-model="settingsForm.reveal_answers"
                                    type="checkbox"
                                    class="rounded"
                                />
                                Reveal answers after submission</label
                            >
                            <div class="flex justify-end md:col-span-2">
                                <button
                                    type="submit"
                                    class="inline-flex items-center gap-2 rounded-lg bg-[#381998] px-4 py-2 text-sm font-semibold text-white hover:bg-[#000928]"
                                >
                    ...
                    >
                        ...
                        <form
                            class="space-y-3 rounded-lg border border-dashed border-gray-300 p-4 dark:border-gray-600"
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                <div
                                    v-for="(_, i) in questionForm.options"
                                    :key="i"
                                ...
                                >
                                    <input
                                        ...
                                        type="text"
                                        class="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100"
                                    ...
                                    <input
                                        ...
                                        :checked="
                                            ...
                                            questionForm.correct_answer.includes(
                                                i,
                                            )
                                        "
                                        @change="
                                ...
                                <button
                                    type="button"
                                    class="text-xs font-medium text-[#381998]"
                            ...
                            <div class="flex gap-2">
                                <button
                                    type="submit"
                                    class="inline-flex items-center gap-2 rounded-lg bg-[#42b6c5] px-4 py-2 text-sm font-semibold text-white hover:bg-[#35919e]"
                                >
                    ...
                    <div
                        v-else
                        class="rounded-xl border border-dashed border-gray-300 bg-white p-6 text-sm text-gray-600 shadow dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300"
                    >
                ...
                <div class="space-y-4">
                    <div
                        class="rounded-xl bg-white p-5 shadow dark:bg-gray-800"
                    >
                        <h3
                        ...
                        >
                            ...
                            <li>
                                Multiple select (more than one correct option)
                            </li>
                            <li>True/False</li>
