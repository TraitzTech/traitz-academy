...
const props = defineProps<{
  ...
  filters: { search?: string | null; course?: number | null; status?: string | null }
}>()
...
watch([courseId, status], applyFilters)
...
<template>
  <div>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">My Students</h2>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Learners enrolled in your courses ({{ enrollments.total }} enrollment{{ enrollments.total === 1 ? '' : 's' }})
        </p>
      </div>
      <Link
        href="/tutor/courses"
        class="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800"
      >
        <BookOpen class="h-4 w-4" /> My courses
      </Link>
    </div>
    ...
    <div class="overflow-hidden rounded-xl bg-white shadow dark:bg-gray-800">
      <div v-if="enrollments.data.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p class="font-medium text-gray-500 dark:text-gray-400">
          {{ search || courseId || status ? 'No enrollments match your filters' : 'No students yet' }}
        </p>
        <p class="mt-1 text-sm text-gray-400">
          Enrollments appear when learners join your published courses.
        </p>
      </div>
