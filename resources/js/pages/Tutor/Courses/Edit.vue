...
function saveDetails() {
    detailsForm.put(`/tutor/courses/${props.course.id}`, {
        preserveScroll: true,
    });
}
...
function submitForReview() {
    router.post(
        `/tutor/courses/${props.course.id}/submit`,
        {},
        {
            preserveScroll: true,
            onFinish: () => {
...
const expandedSections = ref<Set<number>>(
    new Set(props.course.sections.map((s) => s.id)),
);
...
function deleteSection(sectionId: number) {
    ...
    router.delete(`/tutor/courses/${props.course.id}/sections/${sectionId}`, {
        preserveScroll: true,
    });
}
...
function onSectionsDragEnd() {
    ...
    if (order.length === prev.length && order.every((id, i) => id === prev[i]))
        return;
    router.post(
        `/tutor/courses/${props.course.id}/sections/reorder`,
        { order },
        { preserveScroll: true },
    );
}
...
const pendingLessonSave = ref<{ sectionId: number; lessonId: number } | null>(
    null,
);
...
function addLesson(sectionId: number) {
    ...
    const fileToUpload = lessonForm.video_file;
    lessonForm.video_file = null;

    lessonForm.post(
        `/tutor/courses/${props.course.id}/sections/${sectionId}/lessons`,
        {
            preserveScroll: true,
            onSuccess: (page) => {
                lessonForm.reset();
                ...
                const newLesson = section.lessons.reduce((best, l) =>
                    l.id > best.id ? l : best,
                );
...
function saveLesson(sectionId: number, lessonId: number) {
    ...
    if (
        ...
        shouldConfirmVideoReplacement(sectionId, lessonId)
    ) {
        pendingLessonSave.value = { sectionId, lessonId };
        showReplaceVideoModal.value = true;
        return;
    }

    performSaveLesson(sectionId, lessonId);
}

function confirmReplaceVideo() {
    const pending = pendingLessonSave.value;
    showReplaceVideoModal.value = false;
    pendingLessonSave.value = null;
    if (pending) {
        performSaveLesson(pending.sectionId, pending.lessonId);
    }
}

function performSaveLesson(sectionId: number, lessonId: number) {
    const wasVideo = lessonForm.type === 'video';
    const fileToUpload = lessonForm.video_file;

    // The file is attached through the dedicated video endpoint, not the lesson update.
    lessonForm.video_file = null;

    lessonForm.put(
        `/tutor/courses/${props.course.id}/sections/${sectionId}/lessons/${lessonId}`,
        {
            preserveScroll: true,
            onSuccess: () => {
                lessonForm.reset();
...
function deleteLessonAttachment(
    section: Section,
    lesson: Lesson,
    attachmentId: number,
) {
...
function submitEnrollStudent() {
    enrollForm.post(`/tutor/courses/${props.course.id}/enroll-student`, {
        preserveScroll: true,
    });
}
...
function shouldConfirmVideoReplacement(
    sectionId: number,
    lessonId: number,
): boolean {
...
onMounted(async () => {
    ...
    if (highlightedLessonId.value !== null) {
        ...
        const el = document.getElementById(
            `lesson-row-${highlightedLessonId.value}`,
        );
...
<template>
    ...
    >
        ...
        <div class="mb-6 flex items-center gap-4">
            ...
            <div class="min-w-0 flex-1">
                <div class="flex items-center gap-3">
                    <h1
                        class="truncate text-xl font-bold text-[#000928] dark:text-white"
                    >
                        {{ course.title }}
                    </h1>
                    <span
        ...
        <!-- Tab bar -->
        <div
            class="mb-6 flex w-fit gap-1 rounded-xl border border-gray-200 bg-white p-1 shadow-sm dark:border-gray-700 dark:bg-gray-800"
        >
            <button
                v-for="tab in [
        ...
        <!-- ═══════════════════ TAB 1: DETAILS ═══════════════════ -->
        <div v-show="activeTab === 'details'" class="mx-auto max-w-4xl">
            <form
            ...
            >
                ...
                <div class="space-y-8 p-6 sm:p-8">
                    ...
                    <section class="space-y-5">
                        <div>
                            <h3
                                class="text-xs font-bold tracking-wider text-[#381998] uppercase"
                            >
                                Basics
                            </h3>
                            <p
                            ...
                            >
                                Shown on course cards and the public catalogue.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="text"
                                class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                                :class="{
                                    'border-red-400': detailsForm.errors.title,
                                }"
                            />
                            <p
                        ...
                        <div class="grid gap-4 sm:grid-cols-3">
                            <div>
                                ...
                                <select
                                    ...
                                    class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-700 focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                                >
                    ...
                    <section class="space-y-4">
                        <div>
                            ...
                            <p
                                class="mt-1 text-xs text-gray-500 dark:text-gray-400"
                            >
                                Public price, an optional sale price, and how
                                many installments learners can split it into.
                            </p>
                        </div>
                        ...
                        <div class="grid gap-4 sm:grid-cols-3">
                            <div>
                                ...
                                />
                                <p
                                    v-if="detailsForm.errors.price"
                                    class="mt-1 text-xs text-red-500"
                                >
                                    {{ detailsForm.errors.price }}
                                </p>
                            </div>
                            <div>
                                <label
                                    class="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-200"
                                    >Sale price (XAF)</label
                                >
                                <input
                                ...
                                />
                                <p
                                    v-if="detailsForm.errors.sale_price"
                                    class="mt-1 text-xs text-red-500"
                                >
                                    {{ detailsForm.errors.sale_price }}
                                </p>
                            </div>
                            <div>
                                <label
                                ...
                                <input
                                    v-model.number="
                                        detailsForm.max_installments
                                    "
                                    type="number"
                                    min="1"
                                    max="12"
                                    :disabled="
                                        Number(detailsForm.price || 0) <= 0
                                    "
                                    class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-800 focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none disabled:cursor-not-allowed disabled:bg-gray-50 disabled:opacity-60 dark:border-gray-600 dark:bg-gray-900 dark:text-white dark:disabled:bg-gray-800"
                                />
                                <p
                                    v-if="detailsForm.errors.max_installments"
                                    class="mt-1 text-xs text-red-500"
                                >
                                    {{ detailsForm.errors.max_installments }}
                                </p>
                            </div>
                        </div>
                    ...
                    <section class="space-y-5">
                        <div>
                            ...
                            <p
                                class="mt-1 text-xs text-gray-500 dark:text-gray-400"
                            >
                                A one-line summary plus the full overview on the
                                course page.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            >
                            <textarea
                                v-model="detailsForm.short_description"
                                rows="3"
                                maxlength="500"
                                placeholder="A brief summary of what students will learn — shown on course cards."
                                class="w-full resize-none rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                                :class="{
                                    'border-red-400':
                                        detailsForm.errors.short_description,
                                }"
                            />
                            <div class="mt-1 flex items-center justify-between">
                                <p
                                    v-if="detailsForm.errors.short_description"
                                ...
                                <p
                                    ...
                                    :class="
                                        detailsForm.short_description.length >
                                        450
                                            ? 'text-amber-500'
                                            : 'text-gray-400'
                                    "
                        ...
                        <div>
                            ...
                            >
                                Rich text: headings, lists, images, and links —
                ...
                <!-- Footer -->
                <div
                    class="flex justify-end border-t border-gray-100 bg-gray-50/60 px-6 py-4 sm:px-8 dark:border-gray-700 dark:bg-gray-900/30"
                >
                    <button
                        type="submit"
        ...
        >
                ...
                <template #item="{ element: section }">
                    ...
                    >
                            ...
                            <template v-if="editingSectionId === section.id">
                                ...
                                >
                                    <input
                                        ...
                                        class="w-full min-w-0 rounded-xl border border-gray-200 px-3 py-2 text-sm focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/15 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                                    />
                                    <div
                                        class="flex shrink-0 justify-end gap-2 sm:justify-start"
                                    >
                                        <button
                            ...
                            <template v-else>
                                ...
                                >
                                    <button
                                        ...
                                        class="rounded-lg p-1.5 text-gray-400 transition-colors hover:text-[#381998]"
                                    >
                                        <Edit2 class="h-4 w-4" />
                                    </button>
                                    <button
                                        @click="deleteSection(section.id)"
                                        class="rounded-lg p-1.5 text-gray-400 transition-colors hover:text-red-500"
                                    >
                        ...
                        >
                                    ...
                                    >
                                        ...
                                        <div
                                            v-if="
                                                ...
                                                editingLessonSection ===
                                                    section.id
                                            "
                                            class="border-t border-gray-100 bg-gray-50 px-5 py-5 sm:px-6 dark:border-gray-700 dark:bg-gray-900/40"
                                        >
                                            <p
                                                class="mb-4 text-xs font-bold tracking-wide text-gray-500 uppercase dark:text-gray-400"
                                            >
                                                Edit lesson
                                            </p>
                                            <div
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <div
                                                        class="flex flex-wrap items-center gap-3 border-t border-gray-200/80 pt-4 dark:border-gray-600/60"
                                                    >
                                                        <button
                                                            type="submit"
                                                            :disabled="
                                        ...
                                        <div
                                            ...
                                            :class="[
                                                'flex items-center gap-3 px-5 py-2.5 transition-colors sm:px-6',
                                                highlightedLessonId ===
                                                lesson.id
                                                    ? 'bg-[#381998]/8 ring-1 ring-[#381998]/25'
                                                    : '',
                                            ]"
                                        >
                                            <button
                                            ...
                                            <div class="min-w-0 flex-1">
                                                ...
                                                >
                                                    ...
                                                    <span
                                                        v-if="lesson.is_free"
                                                        class="rounded-full bg-green-100 px-1.5 py-0.5 font-medium text-green-600"
                                                        >Free preview</span
                                                    >
                                                </div>
                                            </div>
                                        ...
                                        >
                                            ...
                                            <span
                                                v-if="lesson.youtube_error"
                                                class="max-w-[45ch] truncate text-red-500"
                                                :title="lesson.youtube_error"
                                            >
                                        ...
                                        <div
                                            v-if="
                                                lesson.type === 'video' &&
                                                lessonEmbedUrl(lesson)
                                            "
                                            class="border-t border-gray-50 px-5 pt-2 pb-3 sm:px-6 dark:border-gray-700/50"
                                        >
                                            <div
                                                class="yt-crop-shell aspect-video overflow-hidden rounded-lg bg-black"
                                            >
                                                <iframe
                                                    :src="
                                                        lessonEmbedUrl(
                                        ...
                                        >
                                            <p
                                                class="mb-2 flex items-center gap-2 text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400"
                                            >
                                                <Paperclip
                                            ...
                                            >
                                                Supplementary downloads (PDFs,
                                                slides, worksheets) — shown to
                                                learners in the Resources panel.
                                            </p>
                                            <ul
                                                v-if="
                                                    (lesson.attachments ?? [])
                                            ...
                                            >
                                                <label
                                                    class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-gray-200 px-3 py-2 text-xs font-medium text-gray-600 hover:border-[#42b6c5] dark:border-gray-600 dark:text-gray-300"
                                                >
                                                    <Upload
                                                        class="h-3.5 w-3.5 shrink-0"
                                                    />
                            ...
                            <div
                                v-if="addLessonSectionId === section.id"
                                class="border-t border-gray-100 bg-gray-50 px-5 py-5 sm:px-6 dark:border-gray-700 dark:bg-gray-900/40"
                            >
                                <p
                                    class="mb-4 text-xs font-bold tracking-wide text-gray-500 uppercase dark:text-gray-400"
                                ...
                                <div class="mx-auto w-full max-w-4xl">
                                    <form
                                        @submit.prevent="addLesson(section.id)"
                                        class="space-y-5"
                                    >
                                        <LessonForm :form="lessonForm" />
                                        <p
                                            v-if="
                                                videoUploadForm.errors
                                                    .video_file
                                            "
                                            class="text-xs text-red-500"
                                        >
                            ...
                            <div
                                v-else
                                class="border-t border-gray-50 px-5 py-3.5 sm:px-6 dark:border-gray-700/50"
                            >
                                <button
                                    @click="openAddLesson(section.id)"
                                    class="flex items-center gap-2 text-sm font-medium text-[#381998] transition-colors hover:text-[#000928]"
                                >
                                    <PlusCircle class="h-4 w-4 shrink-0" /> Add
                                    lesson
                                </button>
                            </div>
                        </div>
            ...
            <div
                v-if="addSectionOpen"
                class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:p-7 dark:border-gray-700 dark:bg-gray-800"
            >
                <p
                    class="mb-1 text-base font-bold text-[#000928] dark:text-white"
                >
                    New section
                </p>
                <p class="mb-5 text-sm text-gray-500 dark:text-gray-400">
                    Sections group your lessons. You can reorder them anytime.
                </p>
                <form
                ...
                >
                    <div>
                        ...
                        <input
                            ...
                            class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/15 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                            :class="{
                                'border-red-400': sectionForm.errors.title,
                            }"
                        />
                        <p
                    ...
                    <div>
                        ...
                        <input
                            ...
                            type="text"
                            placeholder="Optional — shown to you when organizing"
                            class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/15 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                        />
                    </div>
                    <div
                        class="flex flex-wrap items-center gap-3 border-t border-gray-100 pt-5 dark:border-gray-700"
                    >
                        <button
                            type="submit"
                            :disabled="sectionForm.processing"
        ...
        >
            ...
            >
                <div
                    class="h-1 bg-gradient-to-r from-[#381998] via-[#42b6c5] to-[#000928]"
                ></div>
                <div class="p-6">
                    <div class="mb-4 flex items-center gap-3">
                        <div
                            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#42b6c5]/10"
                        >
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                The banner students see on the catalogue and
                                course page.
                            </p>
                        </div>
                    </div>
                    ...
                    <div
                        class="mb-4 aspect-[16/6] overflow-hidden rounded-xl border border-gray-100 bg-gradient-to-br from-[#381998] to-[#42b6c5] dark:border-gray-700"
                    >
                        <img
                            v-if="coverPreview"
                            :src="coverPreview"
                    ...
                    <form
                        @submit.prevent="uploadCover"
                        class="flex items-center gap-3"
                    >
                        <label
                            class="flex flex-1 cursor-pointer items-center gap-2 rounded-xl border border-dashed border-gray-300 px-4 py-2.5 text-sm text-gray-600 transition-colors hover:border-[#42b6c5] hover:bg-[#42b6c5]/[0.04] hover:text-[#42b6c5] dark:border-gray-600 dark:text-gray-300"
                        >
                            <ImageIcon class="h-4 w-4 shrink-0" />
                            <span class="truncate">{{
                                coverForm.cover_image
                                    ? coverForm.cover_image.name
                                    : 'Choose image (JPG, PNG, WebP — max 2 MB)'
                            }}</span>
                            <input
                                type="file"
            ...
            >
                ...
                <div class="p-6">
                    <div class="mb-4 flex items-center gap-3">
                        <div
                            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#381998]/10"
                        >
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                Send your course to an admin to be approved and
                                published.
                            </p>
                        </div>
                    </div>
                    ...
                    <!-- Status description -->
                    <div
                        class="mb-5 flex items-center gap-3 rounded-xl bg-gray-50 p-4 dark:bg-gray-900/40"
                    >
                        <div class="flex-1">
                            <p
                            ...
                            <p class="mt-0.5 text-xs text-gray-400">
                                <template v-if="course.status === 'draft'"
                                    >Your course is a draft. Complete the
                                    checklist below and submit for
                                    review.</template
                                >
                                <template
                                    v-else-if="
                                    ...
                                    "
                                    >Awaiting admin review. You'll be notified
                                    once it's approved.</template
                                >
                                <template
                                    v-else-if="course.status === 'published'"
                                    >Your course is live and visible to
                                    students.</template
                                >
                                <template
                                    v-else-if="course.status === 'archived'"
                                    >Archived. You can re-submit for
                                    review.</template
                                >
                            </p>
                        </div>
                    ...
                    <ul class="mb-6 space-y-2">
                        <li
                            v-for="item in [
                                {
                                    label: 'Title and short description filled',
                                    done:
                                        !!course.title &&
                                ...
                                },
                                {
                                    ...
                                    done: !!course.cover_image,
                                },
                                {
                                    label: 'At least one section added',
                                    done: course.sections.length > 0,
                                },
                                {
                                    label: 'At least one lesson added',
                                    done: course.sections.some(
                    ...
                    <p
                        v-else-if="course.status === 'pending_review'"
                ...
                <div class="p-6">
                    <div class="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                Manually grant a registered learner access by
                                email.
                            </p>
                        </div>
                    </div>
                    <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">
                        Grant access to your published course by email. The
                        learner must already have a registered account with that
                        email.
                    </p>
                    <form
                        class="space-y-3"
                        @submit.prevent="submitEnrollStudent"
                    >
                        <div>
                            <label
                            ...
                            <input
                                ...
                                class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                            />
                            <p
    ...
    <ConfirmationModal
        ...
        @update:open="
            (v) => {
                showReplaceVideoModal = v;
                if (!v) pendingLessonSave = null;
            }
        "
