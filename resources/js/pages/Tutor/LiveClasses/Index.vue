  ...
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500">Create and run your live sessions.</p>
      </div>
      <Link href="/tutor/live-classes/create" class="rounded-lg bg-[#381998] px-4 py-2 text-sm font-semibold text-white">Create live class</Link>
    </div>
    ...
    <div v-else class="overflow-hidden rounded-xl bg-white shadow dark:bg-gray-800">
      <table class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
          <tr v-for="row in classes" :key="row.id">
            ...
            <td class="px-4 py-3 capitalize">{{ row.access_type }}</td>
            <td class="px-4 py-3 text-right">
              <Link :href="`/tutor/live-classes/${row.id}`" class="mr-2 text-xs font-semibold text-[#381998]">View</Link>
              <Link :href="`/tutor/live-classes/${row.id}/edit`" class="mr-2 text-xs font-semibold text-[#381998]">Edit</Link>
