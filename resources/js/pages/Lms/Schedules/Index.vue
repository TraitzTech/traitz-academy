...
function submitPersonalEvent() {
    if (editingEventId.value) {
        form.put(
            `/dashboard/schedules/personal-events/${editingEventId.value}`,
            {
                preserveScroll: true,
                onSuccess: () => {
                    resetForm();
...
function deletePersonalEvent(row: ScheduleRow) {
    if (!row.personal_event_id) return;
    form.delete(
        `/dashboard/schedules/personal-events/${row.personal_event_id}`,
        {
            preserveScroll: true,
            onSuccess: () => {
                selectedEvent.value = null;
...
function syncGoogleCalendar() {
    router.post(
        '/dashboard/schedules/google/sync',
        {},
        { preserveScroll: true },
    );
}
...
<template>
    <div>
        ...
        <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Assignments, live classes, LMS events, and your personal
                    events in one calendar.
                </p>
            </div>
            <button
                type="button"
        ...
        <div
            class="mb-5 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800"
        >
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        <span v-if="googleConnected"
                            >Connected as
                            {{ googleEmail || 'Google account' }}</span
                        >
                        <span v-else
                            >Connect your Google account to sync this full
                            schedule.</span
                        >
                    </p>
                </div>
        ...
        <div class="mb-4 flex flex-wrap gap-4 text-xs">
            <span
                class="flex items-center gap-1.5 text-gray-600 capitalize dark:text-gray-400"
            >
            ...
            >
                ...
                >
                    ...
                    <div class="flex gap-1">
                        <button
                            type="button"
                            class="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                            @click="prevMonth"
                        >
                            <ChevronLeft :size="16" />
                        </button>
                ...
                <div
                    class="grid grid-cols-7 border-b border-gray-100 dark:border-gray-700"
                >
                    <div
                        v-for="day in dayNames"
                        :key="day"
                        class="px-1 py-2 text-center text-xs font-semibold text-gray-400 uppercase"
                    >
                        {{ day }}
                    </div>
                </div>
                ...
                <div class="grid grid-cols-7">
                    <div
                        ...
                        :key="i"
                        class="min-h-[84px] border-r border-b border-gray-50 p-1.5 dark:border-gray-700/50"
                        :class="
                            day &&
                            day === now.getDate() &&
                            currentMonth === now.getMonth() &&
                            currentYear === now.getFullYear()
                                ? 'bg-[#381998]/5 dark:bg-[#381998]/10'
                                : ''
                        "
                    >
                        <p
                            v-if="day"
                            class="mb-1 flex h-5 w-5 items-center justify-center rounded-full text-xs font-medium"
                            :class="
                                day === now.getDate() &&
                                currentMonth === now.getMonth() &&
                                currentYear === now.getFullYear()
                ...
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div class="flex min-w-0 items-start gap-3">
                        <span
                            ...
                            :class="typeColor(selectedEvent.source_type)"
                        />
                        <div class="min-w-0">
                            <p
                                class="text-base font-semibold text-gray-900 dark:text-gray-100"
                            ...
                            >
                                <span class="capitalize">{{
                                    sourceLabel(selectedEvent.source_type)
                                }}</span>
                                <span v-if="selectedEvent.attachable.title">
                                    · {{ selectedEvent.attachable.title }}</span
                                >
                            </p>
                            <dl
        ...
        <Teleport to="body">
            ...
            >
                <div
                    class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-800"
                >
                    <div
                        class="sticky top-0 flex items-center justify-between border-b border-gray-100 bg-white px-5 py-4 dark:border-gray-700 dark:bg-gray-800"
                    >
                        <h2
                            id="schedule-event-modal-title"
                    ...
                    <form
                        class="space-y-4 p-5"
                        @submit.prevent="submitPersonalEvent"
                    >
                        <div>
                            <label
                                for="evt-title"
                            ...
                            <input
                                ...
                                class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-[#381998] focus:ring-2 focus:ring-[#381998]/20 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100"
                            />
                            <p
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input
                                    ...
                                    type="datetime-local"
                                    required
                                    class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-[#381998] focus:ring-2 focus:ring-[#381998]/20 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100"
                                />
                                <p
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                class="w-full resize-y rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-[#381998] focus:ring-2 focus:ring-[#381998]/20 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100"
                            />
                            <p
                        ...
                        <div
                            class="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-4 dark:border-gray-700"
                        >
                            <button
                                v-if="editingEventId"
                                type="button"
