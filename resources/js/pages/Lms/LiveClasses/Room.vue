...
const props = defineProps<{ liveClass: any; authUser: { id: number; name: string; role: string }; canManage: boolean; manageRedirectUrl?: string | null }>()
...
function setupJitsi() {
  ...
  const options: any = {
    roomName: props.liveClass?.jitsi?.room_name || props.liveClass.room_name,
...
function jitsiApiScriptUrl(): string {
  const rawDomain = String(props.liveClass?.jitsi?.domain || 'meet.jit.si').trim()
...
onMounted(async () => {
  ...
  try {
    await postJson(`/dashboard/live-classes/${props.liveClass.id}/attendance/join`)
  } catch {
    window.location.href = `/dashboard/live-classes/${props.liveClass.id}/details`
    return
  }
  heartbeat = window.setInterval(() => {
    postJson(`/dashboard/live-classes/${props.liveClass.id}/attendance/ping`).catch(() => {
      window.location.href = `/dashboard/live-classes/${props.liveClass.id}/details`
    })
  }, 30000)
  chat.start()
...
async function toggleRecording() {
  ...
  try {
    ...
    recordingNotice.value = 'Recording action failed. Please check Jitsi recording permissions.'
  } finally {
      ...
      <div v-if="canManage" class="mt-3 flex flex-wrap items-center gap-2">
        <button
          ...
          :class="[
            ...
            recording ? 'bg-red-600 hover:bg-red-700' : 'bg-[#381998] hover:bg-[#000928]',
          ]"
          @click="toggleRecording"
        >
          {{ recordingBusy ? 'Please wait...' : recording ? 'Stop recording' : 'Start recording' }}
        </button>
        <span class="text-xs text-gray-500">{{ recording ? 'Recording is active.' : 'Recording is currently off.' }}</span>
      </div>
      <p v-if="recordingNotice" class="mt-2 text-xs text-gray-500">{{ recordingNotice }}</p>
    </div>
