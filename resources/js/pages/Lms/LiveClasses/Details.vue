...
function availabilityLabel(): string {
    ...
    if (classWindowState() === 'in_window' && !props.hostOnline)
    ...
    <div class="mx-auto max-w-4xl space-y-6">
        ...
        >
            <Link
                ...
                class="mb-4 inline-flex rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700"
            >
            ...
            <div
                class="mt-6 rounded-xl border border-dashed border-gray-300 p-4 text-sm text-gray-600 dark:border-gray-600 dark:text-gray-300"
            >
                <template v-if="canJoinNow">
                    <p>You can now join this class.</p>
                    <p v-if="isMeet" class="mt-1 text-xs text-gray-500">
                        The class runs on Google Meet — it opens in a new tab.
                        If the call ends after ~60 minutes, just click Join
                        again to rejoin the same meeting.
                    </p>
                </template>
                <p v-else-if="classWindowState() === 'ended'">
                    This class has ended. You can no longer join the live
                    session.
                </p>
                <p
                    v-else-if="
                    ...
                    "
                >
                    Waiting for tutor/admin to start the room. Join opens
                    automatically once the host is inside.
                </p>
                <p v-else>
                    Join opens at {{ new Date(joinOpensAt).toLocaleString() }}.
                    {{ startsInText() }}
                </p>
            </div>

            <div
                v-if="classWindowState() === 'ended'"
                class="mt-6 rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm dark:border-gray-600 dark:bg-gray-900/30"
            >
                <h2
                ...
                <div
                    class="grid gap-2 text-gray-600 md:grid-cols-2 dark:text-gray-300"
                >
                    <p>
                        <span
                ...
                <div class="mt-4 space-y-2">
                    ...
                    >
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <span
                                class="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-semibold text-gray-600 dark:bg-gray-700 dark:text-gray-200"
                                >{{ recording.status || 'uploaded' }}</span
                            >
                        </div>
                        <a
            ...
            <div class="mt-6 flex flex-wrap items-center gap-3">
                ...
                <a
                    ...
                    :class="[
                        ...
                        canJoinNow && meetingUrl
                            ? 'bg-[#381998] text-white hover:bg-[#000928]'
