...
<template>
  <div>
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Recorded live classes</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Watch recordings from your completed live sessions.</p>
      </div>
      <Link href="/dashboard/live-classes" class="rounded-lg border border-gray-300 px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">
      ...
      >
        <div class="mb-3">
          ...
          <p class="text-xs text-gray-500">
            Tutor: {{ row.tutor?.name || '—' }} · {{ new Date(row.start_time).toLocaleString() }} · {{ row.duration }} min
          </p>
        </div>
