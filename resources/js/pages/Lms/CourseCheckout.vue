...
<template>
  <div class="lms-page">
    ...
    <div class="mb-8">
      <Link :href="`/dashboard/courses/${course.id}`" class="mb-4 inline-flex items-center text-[#42b6c5] hover:text-[#35919e]">
      ...
      <p class="lms-subtitle">Secure mobile money payment powered by MeSomb</p>
    </div>

    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div class="lms-panel lg:col-span-2">
        <h3 class="mb-4 text-xl font-semibold text-gray-900 dark:text-gray-100">Payment details</h3>

        <form class="space-y-5" @submit.prevent="submit">
          <div>
            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Mobile money number</label>
            <input
          ...
          <div>
            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Payment mode</label>
            <div class="space-y-3">
              <label
                class="block cursor-pointer rounded-lg border border-gray-300 p-4 dark:border-gray-600"
                :class="form.payment_mode === 'full' ? 'border-[#42b6c5] ring-2 ring-[#42b6c5]' : ''"
              >
                <input v-model="form.payment_mode" type="radio" value="full" class="mr-2" />
                <span class="font-semibold text-gray-900 dark:text-gray-100">Pay full remaining balance</span>
                <p class="lms-subtitle">
                  Pay
                  {{ formatMoney(summary.remaining_amount + (summary.remaining_amount * summary.online_surcharge_percentage) / 100) }}
                  now (including {{ summary.online_surcharge_percentage }}% surcharge).
                </p>
              </label>
              ...
              <label
                ...
                class="block cursor-pointer rounded-lg border border-gray-300 p-4 dark:border-gray-600"
                :class="form.payment_mode === 'installment' ? 'border-[#42b6c5] ring-2 ring-[#42b6c5]' : ''"
              >
                <input
                ...
                <p class="lms-subtitle">
                  ...
                  •
                  {{ formatMoney(installmentDue + (installmentDue * summary.online_surcharge_percentage) / 100) }}
                  including surcharge
                </p>
              </label>
            </div>
      ...
      <div class="lms-panel h-fit">
        <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Payment summary</h3>
        <div class="space-y-3 text-sm">
          <div class="flex justify-between gap-4">
            <span class="text-gray-500 dark:text-gray-400">Course</span>
            <span class="text-right font-medium text-gray-900 dark:text-gray-100">{{ course.title }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500 dark:text-gray-400">Total fee</span>
            <span class="font-medium text-gray-900 dark:text-gray-100">{{ formatMoney(summary.course_price) }}</span>
          </div>
          <div class="flex justify-between">
          ...
          <div class="flex justify-between border-t border-gray-200 pt-3 dark:border-gray-700">
            <span class="font-medium text-gray-700 dark:text-gray-300">You pay now</span>
