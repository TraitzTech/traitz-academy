...
const canTakeTimestampNote = computed(() =>
    Boolean(youtubeId.value || nativeVideoSrc.value),
);
...
const currentIndex = computed(() =>
    flatLessons.value.findIndex((lsn) => lsn.id === props.lesson.id),
);
const nextLesson = computed(() => {
    if (
        currentIndex.value < 0 ||
        currentIndex.value >= flatLessons.value.length - 1
    )
        return null;
...
function deleteDiscussion(discussionId: number) {
    ...
    router.delete(`${discussionsBasePath()}/${discussionId}`, {
        preserveScroll: true,
    });
}
...
function queueSaveVideoProgress(
    watchedSeconds: number,
    durationSeconds: number,
) {
    if (durationSeconds <= 0 || postingProgress.value) return;
    if (watchedSeconds <= lastPostedSecond.value && watchedSeconds !== 0)
        return;
...
async function saveVideoProgress(
    ...
    try {
        const response = await fetch(
            `/dashboard/courses/${props.course.id}/lessons/${props.lesson.id}/progress`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
        ...
        progressPercent.value =
            payload.progressPercent ?? progressPercent.value;
...
async function takeTimestampNote() {
    if (!canTakeTimestampNote.value) {
        noteMessage.value =
            'Timestamp capture is currently available for YouTube and uploaded videos.';
        openNotesPanel();
        return;
    }
    ...
    lessonNoteDraft.value =
        lessonNoteDraft.value.trim() === ''
            ? `[${stamp}] `
            : `${lessonNoteDraft.value}\n[${stamp}] `;
...
async function markCurrentLessonComplete() {
    const response = await fetch(
        `/dashboard/courses/${props.course.id}/lessons/${props.lesson.id}/complete`,
        {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    <div>
        ...
        <div class="lms-page">
            <section class="bg-gray-50 py-6">
                <div
                    class="mx-auto grid max-w-7xl gap-6 px-4 lg:grid-cols-[320px_1fr]"
                >
                    <aside class="lms-panel p-4">
                        <h2
                            class="line-clamp-2 text-sm font-bold text-[#000928]"
                        >
                            {{ course.title }}
                        </h2>
                        <div class="mt-3">
                            ...
                            <div class="h-2 rounded-full bg-gray-100">
                                <div
                                    class="h-2 rounded-full bg-[#42b6c5]"
                        ...
                        <div
                            class="mt-4 max-h-[70vh] space-y-3 overflow-auto pr-1"
                        >
                            <div
                            ...
                            >
                                <div
                                    class="border-b border-gray-100 bg-gray-50 px-3 py-2 text-xs font-semibold text-[#000928]"
                                >
                                    {{ section.title }}
                                </div>
                                <Link
                    ...
                    <main class="lms-panel">
                        ...
                        >
                            <div>
                                <p
                                    class="text-xs font-semibold tracking-wide text-[#42b6c5] uppercase"
                                >
                                    Course lesson
                                </p>
                                <h1 class="text-2xl font-bold text-[#000928]">
                                    {{ lesson.title }}
                                </h1>
                                <p
                        ...
                        <p v-if="lesson.description" class="mb-5 text-gray-600">
                            {{ lesson.description }}
                        </p>
                        ...
                        <div v-if="lesson.type === 'video'" class="space-y-4">
                            ...
                            <template v-if="youtubeId">
                                <div
                                    ref="ytContainerRef"
                                    class="overflow-hidden rounded-xl border border-gray-200 bg-black shadow-sm"
                                >
                                    <div
                                        class="yt-crop-shell relative aspect-video min-h-[200px] w-full overflow-hidden"
                                    >
                                        <YoutubeIframe
                                            :video-id="youtubeId"
                  ...
                  -->
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            type="button"
                                            class="lms-btn-primary !px-3 !py-1.5"
                                            @click="toggleYoutubePlayback"
                                        >
                                        ...
                                        >
                                            {{
                                                formatDuration(ytCurrentTime)
                                            }}
                                            / {{ formatDuration(ytDuration) }}
                                        </span>
                                    </div>
                                    <div
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            <button
                                                type="button"
                                                class="lms-btn-outline !px-3 !py-1.5"
                                    ...
                                    <p class="text-xs text-gray-500">
                                        Lesson completes automatically at
                                        {{ VIDEO_COMPLETE_PERCENT }}% watched.
                                    </p>
                                </div>
                            </template>
                            ...
                            >
                                <div
                                    class="aspect-video overflow-hidden rounded-xl border border-gray-200 bg-black shadow-sm"
                                >
                                    <iframe
                                        :src="
                                            streamingEmbedSrc(
                                ...
                                <p class="text-xs text-gray-500">
                                    Use the player’s controls for speed and
                                    volume. Tap
                                    <strong>Mark complete</strong>
                                    when you have finished this lesson (hosted
                                    on Vimeo).
                                </p>
                            </template>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <select
                                            ...
                                            class="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm font-medium text-[#000928] shadow-sm focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none"
                                        >
                                            <option
                            ...
                            <p v-else class="text-sm text-gray-500">
                                No video source is available for this lesson.
                            </p>
                        </div>
                        ...
                        >
                            <div
                                class="lesson-body-text prose prose-lg w-full max-w-3xl leading-relaxed text-gray-800 prose-slate dark:prose-invert prose-headings:scroll-mt-24 prose-headings:font-semibold prose-headings:text-[#000928] prose-p:leading-relaxed prose-a:font-medium prose-a:text-[#381998] prose-a:no-underline hover:prose-a:underline prose-figure:my-6 prose-code:rounded prose-code:bg-gray-100 prose-code:px-1.5 prose-code:py-0.5 prose-code:font-mono prose-code:text-sm prose-code:text-[#381998] prose-code:before:content-none prose-code:after:content-none dark:prose-code:bg-gray-800 dark:prose-code:text-[#7ee8f9] prose-pre:overflow-x-auto prose-pre:rounded-xl prose-pre:border prose-pre:border-gray-800 prose-pre:bg-[#0d1117] prose-pre:px-4 prose-pre:py-3 prose-pre:text-[13px] prose-pre:leading-relaxed prose-pre:text-gray-100 prose-pre:shadow-inner prose-ol:my-4 prose-ul:my-4 prose-li:my-1 prose-img:max-w-full prose-img:rounded-lg prose-img:shadow-sm [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_pre_code]:text-[inherit]"
                                v-html="textLessonHtml"
                            />
                            <div
                                class="w-full max-w-3xl border-t border-gray-100 pt-8"
                            >
                                <button
                                    type="button"
                            ...
                            >
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">
                                        Autosaves as you type. Timestamp notes
                                        jump video playback.
                                    </p>
                                </div>
                                <span
                                    class="text-xs font-semibold text-[#42b6c5]"
                                >
                            ...
                            <div v-if="notesPanelOpen" class="mt-4 space-y-4">
                                <div>
                                    <textarea
                                        ...
                                        class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none"
                        ...
                        >
                            <h3
                                class="mb-3 text-sm font-semibold text-[#000928]"
                            >
                                Resources
                            </h3>
                            <p class="mb-4 text-xs text-gray-500">
                                Download supplementary materials for this lesson
                                (PDFs, slides, worksheets, templates).
                            </p>
                            <ul class="space-y-2">
                                <li
                                ...
                                >
                                    ...
                                    >
                                        <div class="min-w-0">
                                            <p
                                                class="line-clamp-2 font-medium text-gray-800"
                                            >
                                                {{ file.name }}
                                            </p>
                                            <p class="text-xs text-gray-500">
                        ...
                        <section class="mt-8 border-t border-gray-100 pt-8">
                            <div class="mb-4 flex items-center gap-2">
                                ...
                                <h2 class="text-lg font-bold text-[#000928]">
                                    Questions &amp; answers
                                </h2>
                            </div>
                            <p class="mb-4 text-sm text-gray-500">
                                Ask a question about this lesson. Other enrolled
                                learners and your instructor can reply.
                            </p>
                            ...
                            >
                                ...
                                <textarea
                                    ...
                                    class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none"
                                />
                                ...
                                <p
                                    v-if="discussionForm.errors.body"
                                    class="text-xs text-red-600"
                                >
                                    {{ discussionForm.errors.body }}
                                </p>
                            </form>
                            ...
                            <div class="space-y-6">
                                ...
                                >
                                    <div class="flex gap-3">
                                        <div
                                            class="flex flex-col items-center gap-1 pt-0.5"
                                        >
                                            <button
                                                type="button"
                                            ...
                                            >
                                                ...
                                                <span
                                                    ...
                                                    >{{ q.upvotes_count }}</span
                                                >
                                            </button>
                                        </div>
                                        <div class="min-w-0 flex-1">
                                            <p
                                                class="text-sm whitespace-pre-wrap text-gray-800"
                                            >
                                                {{ q.body }}
                                            </p>
                                            <div
                                            ...
                                            <div
                                                v-if="q.replies?.length"
                                                class="mt-4 space-y-3 border-l-2 border-[#42b6c5]/30 pl-4"
                                            >
                                                <div
                                                ...
                                                >
                                                    <p
                                                        v-if="
                                                            r.is_accepted_answer
                                                        "
                                                        class="mb-1 text-xs font-bold tracking-wide text-[#42b6c5] uppercase"
                                                    >
                                                        Accepted answer
                                                    </p>
                                                    <p
                                                    ...
                                                    >
                                                        ...
                                                        <button
                                                            v-if="
                                                                lessonDiscussions.can_moderate &&
                                                                r.can_accept &&
                                                                !r.is_accepted_answer
                                                            "
                                                            type="button"
                                            ...
                                            >
                                                ...
                                                <textarea
                                                    ...
                                                    class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#42b6c5] focus:ring-2 focus:ring-[#42b6c5]/20 focus:outline-none"
                                                />
                                                <div class="flex justify-end">
                                                    <button
                                                        type="submit"
                            ...
                            <p
                                v-if="!lessonDiscussions.questions.length"
                        ...
                        >
                            <Link
                                ...
                                class="text-sm font-semibold text-gray-500 hover:text-[#42b6c5]"
                            >
                                Back to course
                            </Link>
                            <Link
