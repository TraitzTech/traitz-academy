...
const isLoggedIn = computed(
    () => !!(page.props.auth as { user?: unknown })?.user,
);
...
const priceInfo = computed(() => {
    ...
    const sp = props.course.sale_price
        ? parseFloat(props.course.sale_price)
        : null;
    if (p <= 0)
        return { label: 'Free', amount: 0, strike: null as string | null };
    if (sp != null && sp > 0 && sp < p)
        return {
            label: `${formatMoney(sp)} XAF`,
            amount: sp,
            strike: `${formatMoney(p)} XAF`,
        };
    return {
        label: `${formatMoney(p)} XAF`,
        amount: p,
...
const maxInstallments = computed(() =>
    Math.max(1, props.course.max_installments ?? 1),
);

const perInstallmentApprox = computed(() => {
    if (priceInfo.value.amount <= 0) return 0;
    return (
        Math.round((priceInfo.value.amount / maxInstallments.value) * 100) / 100
    );
});
    ...
    <PublicLayout>
        ...
        <div class="lms-page">
            ...
            <div class="border-b border-gray-100 bg-gray-50/80">
                <div
                    class="mx-auto max-w-7xl px-4 py-3 text-sm sm:px-6 lg:px-8"
                >
                    <nav
            ...
            <!-- Hero -->
            <section
                class="relative overflow-hidden bg-linear-to-br from-[#000928] via-[#1a0a52] to-[#381998] text-white"
            >
                <div
                    class="absolute top-0 right-0 h-72 w-72 rounded-full bg-[#42b6c5]/15 blur-3xl"
                />
                <div
                ...
                >
                    <div class="grid gap-10 lg:grid-cols-5 lg:items-start">
                        <div class="lg:col-span-3">
                            ...
                            <h1
                                class="text-3xl leading-tight font-bold sm:text-4xl"
                            >
                                {{ course.title }}
                            </h1>
                            <p class="mt-4 max-w-2xl text-base text-gray-300">
                                {{ course.short_description }}
                            </p>
                            <div
                                class="mt-6 flex flex-wrap items-center gap-6 text-sm text-gray-300"
                            >
                                <span
                                ...
                                <span
                                    ...
                                    class="inline-flex items-center gap-2"
                                >
                                    <Clock class="h-4 w-4 text-[#42b6c5]" />
                                    {{ course.duration }}
                                </span>
                        ...
                        <div
                            class="relative aspect-video overflow-hidden rounded-2xl border border-white/10 shadow-2xl lg:col-span-2"
                        >
                            <img
                            ...
                            <div
                                v-else
                                class="flex h-full min-h-[200px] items-center justify-center bg-white/5"
                            >
                ...
                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div class="grid gap-8 lg:grid-cols-3">
                        ...
                        <div class="space-y-8 lg:col-span-2">
                            ...
                            <template
                                v-if="!isEnrolled || activeTab === 'overview'"
                            >
                                <div
                                ...
                                >
                                    <h2
                                        class="mb-3 flex items-center gap-2 text-lg font-bold text-[#000928]"
                                    >
                                        <BookOpen
                                    ...
                                    <div
                                        class="prose prose-sm max-w-none text-gray-600 dark:text-gray-300 dark:prose-invert prose-headings:text-[#000928] dark:prose-headings:text-gray-100 prose-a:text-[#42b6c5]"
                                        v-html="
                                ...
                                <div class="lms-panel">
                                    ...
                                    >
                                        Curriculum will appear here once
                                        published.
                                    </div>
                                        ...
                                        >
                                            ...
                                            <div
                                                v-show="
                                                    expanded.has(section.id)
                                                "
                                                class="divide-y divide-gray-100 border-t border-gray-100 bg-white"
                                            >
                                                <div
                            ...
                            >
                                <h2
                                    class="mb-2 text-lg font-bold text-[#000928]"
                                >
                                    Course notes
                                </h2>
                                <p class="mb-5 text-sm text-gray-500">
                                    All notes you wrote across lessons, listed
                                    in curriculum order.
                                </p>
                                ...
                                >
                                    No notes yet. Open any lesson and start
                                    writing notes.
                                </div>
                                ...
                                <ul v-else class="space-y-3">
                                    ...
                                    >
                                        ...
                                        >
                                            <span
                                                class="rounded-md bg-gray-100 px-2 py-0.5 font-semibold text-gray-700"
                                                >{{ note.lesson.title }}</span
                                            >
                                            <span
                                                v-if="note.timestamp"
                        ...
                        <!-- Sidebar: pricing -->
                        <div class="space-y-6">
                            <div class="lms-panel sticky top-24">
                                <h3
                                    class="mb-4 text-sm font-bold tracking-wide text-gray-500 uppercase"
                                >
                                    Enrollment
                                </h3>
                                <div class="mb-4">
                                    ...
                                    <template v-else>
                                        <p
                                            class="text-3xl font-bold text-[#000928]"
                                        >
                                            {{ priceInfo.label }}
                                        </p>
                                        <p
                                            v-if="priceInfo.strike"
                                ...
                                <p class="text-center text-xs text-gray-400">
                                    Full access after you enroll.
                                </p>
                            </div>

                            <div
                                v-if="
                                    priceInfo.amount > 0 && maxInstallments > 1
                                "
                                class="lms-panel"
                            >
                                <h3
                                ...
                                <p class="text-sm text-gray-600">
                                    ...
                                    {{ maxInstallments }} installment(s)
                                </p>
                                <p class="mt-1 text-sm text-gray-500">
                                    ≈
                                    {{ formatMoney(perInstallmentApprox) }} XAF
                                    per installment
                                </p>
                            </div>
                            ...
                            <!-- Free previews list -->
                            <div
                                v-if="previewLessons.length > 0"
                                class="rounded-2xl border border-[#42b6c5]/30 bg-[#42b6c5]/5 p-6"
                            >
                                <h3
                                    class="mb-3 flex items-center gap-2 text-sm font-bold text-[#000928]"
                                >
                                    <Check class="h-4 w-4 text-[#42b6c5]" />
                                    Free preview lessons
                                </h3>
                                <ul class="space-y-2">
                                    <li
                                        v-for="pl in previewLessons"
                                        :key="pl.id"
                                    >
                                        <Link
