...
const totalPoints = computed(() =>
    props.quiz.questions.reduce((sum, q) => sum + (q.points ?? 0), 0),
);
...
async function persistProgress() {
    ...
    try {
        const res = await fetch(
            `/dashboard/quizzes/${props.quiz.id}/attempts/${props.attempt.id}/progress`,
            {
                method: 'PATCH',
                headers: {
...
function toggleMulti(
    ...
    const next = checked
        ? [...current, optionIndex]
        : current.filter((x: number) => x !== optionIndex);
...
function submitQuiz() {
    ...
    router.post(
        `/dashboard/quizzes/${props.quiz.id}/attempts/${props.attempt.id}/submit`,
        ...
        <div class="lms-page mx-auto max-w-3xl">
            ...
            <div class="lms-panel">
                <div
                    ...
                    class="mb-6 rounded-xl border border-gray-100 bg-gray-50/90 px-4 py-4 sm:px-5 dark:border-gray-700 dark:bg-gray-900/40"
                >
                    <h2
                    ...
                    <p class="mb-3 text-xs text-gray-500 dark:text-gray-400">
                        Files for this lesson — download before or while you
                        take the quiz.
                    </p>
                    <ul class="space-y-2">
                        <li
                            v-for="file in quiz.lesson.attachments"
                            :key="file.id"
                        >
                            <a
                            ...
                            >
                                <span
                                    class="min-w-0 flex-1 font-medium text-gray-800 dark:text-gray-100"
                                    >{{ file.name }}</span
                                >
                ...
                <p
                    ...
                    class="mb-6 text-sm whitespace-pre-wrap text-gray-600 dark:text-gray-300"
                >
                    {{ quiz.instructions }}
                </p>
                        ...
                        >
                            ...
                            >
                                <input
                                    type="checkbox"
                                    :checked="
                                        Array.isArray(answers[String(q.id)]) &&
                                        answers[String(q.id)].includes(optIndex)
                                    "
                                    @change="
                                        toggleMulti(
                                            q.id,
                        ...
                        >
                            ...
                            >
                                <input
                                    ...
                                    :name="`q_${q.id}`"
                                    value="true"
                                    :checked="answers[String(q.id)] === 'true'"
                                    @change="onAnswerChange(q.id, 'true')"
                                />
                                <span>True</span>
                            </label>
                            <label
                            ...
                            >
                                <input
                                    ...
                                    :name="`q_${q.id}`"
                                    value="false"
                                    :checked="answers[String(q.id)] === 'false'"
                                    @change="onAnswerChange(q.id, 'false')"
                                />
                                <span>False</span>
                            </label>
                        </div>
