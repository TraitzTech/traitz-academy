...
const logbookDone = computed(
    () =>
        !!props.logbook &&
        ['submitted', 'approved'].includes(props.logbook.status),
);
const canClockOut = computed(
    () =>
        clockedIn.value &&
        !clockedOut.value &&
        (!props.requiresLogbookBeforeClockOut || logbookDone.value),
);
...
const elapsedLabel = computed(() => {
    ...
    const minutes = Math.max(
        0,
        Math.floor(
            (now.value - new Date(props.attendance.clock_in_at).getTime()) /
                60000,
        ),
    );
...
const statusBadgeClass = computed(
    () => statusStyles[props.internship.status] ?? statusStyles.withdrawn,
);

const isRemoteInternship = computed(
    () => props.internship.work_mode === 'remote',
);
...
const skippingOffice = ref(
    scheduleConfigured.value ? !props.todayIsOfficeDay : false,
);
...
function clockIn() {
    ...
    if (!navigator.geolocation) {
        toast.error(
            'Your browser does not support location. Ask an admin to check the office is set up.',
        );
        return;
    }
    busy.value = true;
    navigator.geolocation.getCurrentPosition(
        ...
        () => {
            busy.value = false;
            toast.error(
                'Could not get your location. Allow location access (needs HTTPS) and make sure you are at the office.',
            );
        },
...
const logbookStatusClass = computed(
    () =>
        logbookStatusStyles[props.logbook?.status ?? 'draft'] ??
        logbookStatusStyles.draft,
);
</script>
        ...
        >
            <div class="p-6">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <h1
                            class="text-xl font-bold text-[#000928] dark:text-white"
                        >
                            {{ internship.program || 'Internship' }}
                        </h1>
                        <p
                            v-if="internship.cohort"
                            class="mt-0.5 inline-flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400"
                        >
                ...
                <div
                    class="mt-5 grid gap-3 border-t border-gray-100 pt-4 sm:grid-cols-3 dark:border-gray-700"
                >
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span
                            ...
                            >{{
                                internship.supervisor || 'To be assigned'
                            }}</span
                        >
                    </div>
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span
                            ...
                            >{{ fmtDate(internship.start_date) }} →
                            {{ fmtDate(internship.end_date) }}</span
                        >
                    </div>
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        >
                        <span
                            class="font-medium text-gray-800 capitalize dark:text-gray-100"
                            >{{ internship.work_mode }}</span
                        >
                        <span v-if="officeDaysLabel" class="text-gray-400"
                            >· office {{ officeDaysLabel }}</span
                        >
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 sm:p-8">
                ...
                >
                    <div>
                        ...
                        <h2
                            class="text-lg font-bold text-[#000928] dark:text-white"
                        >
                            {{ today }}
                        </h2>
                    </div>
                ...
                <!-- Attendance -->
                <div
                    class="mb-6 rounded-xl border border-gray-100 dark:border-gray-700"
                >
                    <div
                        v-if="isRemoteInternship"
                        class="flex items-center gap-2.5 p-4 text-sm text-gray-600 dark:text-gray-300"
                    >
                        <Laptop class="h-4 w-4 shrink-0 text-[#2a8a96]" />
                        Fully remote internship — no office clock-in needed
                        today.
                    </div>
                    ...
                    <template v-else>
                        ...
                        <div
                            v-if="
                                !clockedIn && !clockedOut && scheduleConfigured
                            "
                        ...
                        >
                            <p class="flex items-center gap-2 text-sm">
                                <component
                                    :is="scheduleIcon"
                                    class="h-4 w-4 shrink-0 text-[#381998]"
                                />
                                <span class="text-gray-600 dark:text-gray-300">
                                    <strong
                                        class="text-[#000928] dark:text-white"
                                        >{{
                                            skippingOffice
                                                ? 'Remote day'
                                                : 'Office day'
                                    ...
                                    >
                                    per your schedule<template
                                        v-if="officeDaysLabel"
                                    >
                                        (office days:
                                        {{ officeDaysLabel }})</template
                                    >
                                </span>
                            </p>
                            <button
                                type="button"
                                class="shrink-0 text-xs font-semibold text-[#381998] hover:underline"
                                @click="skippingOffice = !skippingOffice"
                            >
                                Not today? Switch
                            </button>
                        </div>
                        ...
                        <div
                            v-else-if="!clockedIn && !clockedOut"
                            ...
                            <button
                                ...
                                :class="[
                                    ...
                                    !skippingOffice
                                        ...
                                        : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-900/40',
                                ]"
                                @click="skippingOffice = false"
                            >
                                <Building2 class="h-4 w-4" /> Working from the
                                office
                            </button>
                            <button
                                type="button"
                        ...
                        <div
                            ...
                            :class="{ '!border-t-0': clockedIn || clockedOut }"
                        >
                            <!-- Elsewhere: no timeline needed -->
                            <div
                                v-if="
                                    skippingOffice && !clockedIn && !clockedOut
                                "
                                class="flex items-center gap-2.5 text-sm text-gray-500 dark:text-gray-400"
                            >
                                <Home class="h-4 w-4 shrink-0" /> No clock-in
                                needed today — just submit your logbook below.
                            </div>
                            ...
                            <div v-else class="flex items-center gap-3">
                                <div class="flex flex-1 items-center gap-3">
                                    ...
                                    <div
                                        ...
                                        :class="
                                            clockedOut
                                                ? 'bg-gradient-to-r from-[#381998] to-[#42b6c5]'
                                                : clockedIn
                                                  ? 'bg-[#42b6c5]/30'
                                    ...
                                    >
                                        <span
                                            v-if="
                                                clockedOut && attendance?.hours
                                            "
                                            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-gray-100 bg-white px-2 py-0.5 text-[10px] font-semibold whitespace-nowrap text-gray-500 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400"
                                            >{{ attendance.hours }}h</span
                                        >
                                        <span
                                            v-else-if="
                                                clockedIn && elapsedLabel
                                            "
                                            class="absolute top-1/2 left-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center gap-1 rounded-full border border-[#42b6c5]/30 bg-white px-2 py-0.5 text-[10px] font-semibold whitespace-nowrap text-[#2a8a96] shadow-sm dark:bg-gray-800"
                                        >
                                            <span
                                                class="h-1.5 w-1.5 animate-pulse rounded-full bg-[#42b6c5]"
                                            ></span>
                                            {{ elapsedLabel }}
                                        </span>
                                    </div>
                                    ...
                                    >
                                        <LogOut
                                            class="h-4 w-4 shrink-0 text-[#2a8a96]"
                                        />
                                        <div class="leading-tight">
                                            <p
                                                class="text-[10px] font-semibold tracking-wide text-gray-400 uppercase"
                                ...
                                >
                                    <AlertTriangle class="h-4 w-4" /> Fill
                                    logbook to clock out
                                </Link>
                                <span
                                    v-else
                                    class="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-emerald-100 px-3.5 py-2 text-xs font-semibold text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300"
                                >
                                    <CheckCircle2 class="h-3.5 w-3.5" /> Done
                                    for today
                                </span>
                            </div>
                            ...
                            >
                                <MapPin class="h-3.5 w-3.5 shrink-0" /> We'll
                                check your location when you clock in.
                            </p>
                        </div>
                    </template>
                </div>
                ...
                >
                    ...
                    >
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                >
                                    Required every working day — office or not.
                                </p>
                            </div>
                            <span
                                v-if="logbook"
                            ...
                            <span
                                v-else
                                class="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-700 dark:bg-amber-900/30 dark:text-amber-300"
                            >
                                Not filled yet
                            </span>
                        </div>
                    ...
                    <div
                        v-if="logbook?.supervisor_feedback"
                        class="mt-4 flex gap-2.5 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-200"
                    >
