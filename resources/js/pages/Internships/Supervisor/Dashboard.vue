...
const cards = computed(() => [
    ...
    {
        label: 'Behind on logs',
        value: props.stats.behind,
        icon: AlertTriangle,
        tone: 'text-rose-600',
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <p class="mt-0.5 text-sm text-gray-500">
                Everything across the interns you supervise.
            </p>
        </div>
        ...
        <div class="flex flex-wrap gap-3">
            <Link
                ...
                class="inline-flex items-center gap-2 rounded-xl bg-[#381998] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#000928]"
            >
        ...
        >
            ...
            >
                ...
                <p class="text-xs text-gray-400">
                    Interns with logbooks to review or missed days.
                </p>
            </div>
                ...
                >
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="truncate text-xs text-gray-400">
                            {{ row.program
                            }}<span v-if="row.cohort"> · {{ row.cohort }}</span>
                        </p>
                    </div>
                    <span
                        v-if="row.clocked_in_today"
                        class="hidden shrink-0 items-center gap-1 text-xs text-emerald-600 sm:inline-flex"
                    >
                        <Clock class="h-3 w-3" /> In today
                    </span>
                    <span
                        v-if="row.pending_reviews > 0"
                        class="inline-block shrink-0 rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold whitespace-nowrap text-amber-700 dark:bg-amber-900/30 dark:text-amber-300"
                    >
                        {{ row.pending_reviews }} to review
                    </span>
                    <span
                        v-if="row.missed_logbook_days > 0"
                        class="inline-block shrink-0 rounded-full bg-rose-100 px-2.5 py-0.5 text-xs font-semibold whitespace-nowrap text-rose-700 dark:bg-rose-900/30 dark:text-rose-300"
                    >
