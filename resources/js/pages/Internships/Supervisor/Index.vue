...
const programs = computed(
    () =>
        [
            ...new Set(props.interns.map((i) => i.program).filter(Boolean)),
        ] as string[],
);
const cohorts = computed(
    () =>
        [
            ...new Set(props.interns.map((i) => i.cohort).filter(Boolean)),
        ] as string[],
);
const statuses = computed(() => [
    ...new Set(props.interns.map((i) => i.status)),
]);
...
const filtered = computed(() => {
    ...
    return props.interns.filter((i) => {
        if (q && !`${i.name} ${i.email}`.toLowerCase().includes(q))
            return false;
        if (programFilter.value && i.program !== programFilter.value)
            return false;
    ...
    <div class="mx-auto max-w-6xl">
        ...
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-[#000928] dark:text-white">
                    {{ viewAll ? 'Intern Activity' : 'My Interns' }}
                </h1>
                <p class="text-sm text-gray-500">
                    {{
                        viewAll
                            ? 'Every intern across all cohorts — review logbooks and attendance.'
                            : 'Interns you supervise — review their logbooks and attendance.'
                    }}
                </p>
            </div>
        </div>
        ...
        >
            <div class="relative min-w-[220px] flex-1">
                ...
                <input
                    ...
                    class="w-full rounded-xl border border-gray-200 py-2.5 pr-4 pl-9 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                />
            </div>
            <select
                v-model="programFilter"
                class="rounded-xl border border-gray-200 px-3 py-2.5 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white"
            >
            ...
            >
                ...
                <option v-for="c in cohorts" :key="c" :value="c">
                    {{ c }}
                </option>
            </select>
            <select
                v-model="statusFilter"
                class="rounded-xl border border-gray-200 px-3 py-2.5 text-sm capitalize dark:border-gray-600 dark:bg-gray-900 dark:text-white"
            >
        ...
        >
            ...
            >
                {{
                    viewAll
                        ? 'No interns yet. Accept an internship application to create one.'
                ...
                <table class="w-full table-fixed text-sm">
                    ...
                    >
                        ...
                        >
                            <td class="px-5 py-3">
                                <p
                                    class="truncate font-semibold text-[#000928] dark:text-white"
                                >
                                    {{ i.name }}
                                </p>
                            ...
                            <td
                                class="truncate px-5 py-3 text-gray-600 dark:text-gray-300"
                            >
                                {{ i.program || '—' }}
                            </td>
                            <td
                            ...
                            <td
                                class="truncate px-5 py-3 text-gray-600 capitalize dark:text-gray-300"
                            >
                                {{ i.status }}
                            </td>
                            <td class="px-5 py-3">
                                <span
                                    v-if="i.pending_reviews > 0"
                                    class="inline-block rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold whitespace-nowrap text-amber-700"
                                    >{{ i.pending_reviews }} pending</span
                                >
                            ...
                            <td class="px-5 py-3">
                                <span
                                    v-if="i.missed_logbook_days > 0"
                                    class="inline-block rounded-full bg-rose-100 px-2.5 py-0.5 text-xs font-semibold whitespace-nowrap text-rose-700"
                                    >{{ i.missed_logbook_days }} missed</span
                                >
                                <span v-else class="text-xs text-gray-400"
                                    >—</span
                                >
