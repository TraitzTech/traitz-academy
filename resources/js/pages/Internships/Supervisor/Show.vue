...
interface Compliance {
    ...
    logbook_entries_submitted: number;
...
function review(entryId: number, status: 'approved' | 'needs_revision') {
    router.put(
        `/supervisor/interns/logbook/${entryId}/review`,
        {
            status,
...
function markAttendance(date: string, status: string) {
    router.post(
        `/supervisor/interns/${props.internship.id}/attendance`,
        { date, status },
        {
            preserveScroll: true,
        },
...
<template>
    <div class="mx-auto max-w-6xl space-y-6">
        ...
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-0.5 text-sm text-gray-500">
                    {{ internship.program
                    }}<span v-if="internship.cohort">
                        · {{ internship.cohort }}</span
                    >
                    · {{ internship.email }}
                </p>
            </div>
        ...
        <div class="grid grid-cols-3 gap-4">
            <div
                class="rounded-2xl border border-gray-100 bg-white p-4 text-center shadow-sm dark:border-gray-700 dark:bg-gray-800"
            >
                <p class="text-2xl font-bold text-[#000928] dark:text-white">
                    {{ compliance.working_days_elapsed }}
                </p>
            ...
            >
                <p class="text-2xl font-bold text-emerald-600">
                    {{ compliance.logbook_entries_submitted }}
                </p>
            ...
            >
                <p
                    :class="[
                        'text-2xl font-bold',
                        compliance.missed_logbook_days > 0
                            ? 'text-rose-600'
                            : 'text-gray-400',
                    ]"
                >
                    {{ compliance.missed_logbook_days }}
                </p>
        ...
        >
            <h2 class="mb-4 font-bold text-[#000928] dark:text-white">
                Logbook
            </h2>
            <div v-if="logbook.length === 0" class="text-sm text-gray-500">
            ...
            <div v-else class="space-y-4">
                ...
                >
                    <div class="mb-2 flex items-center justify-between">
                        ...
                        >
                        <span
                            class="rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-600 dark:bg-gray-700 dark:text-gray-200"
                            >{{ statusLabel(e.status) }}</span
                        >
                    </div>
                    <div
                        class="prose prose-sm max-w-none text-gray-700 dark:text-gray-200 dark:prose-invert"
                        v-html="lessonBodyHtml(e.content, 'No entry text.')"
                    />
                    <div
                    ...
                    <p
                        v-if="e.supervisor_feedback"
                        class="mt-2 rounded-lg bg-amber-50 px-3 py-1.5 text-xs text-amber-800 dark:bg-amber-900/20 dark:text-amber-200"
                    >
                        Feedback: {{ e.supervisor_feedback }}
                    </p>
                    ...
                    >
                        <textarea
                            v-model="feedback[e.id]"
                            rows="2"
                            placeholder="Feedback (optional)…"
                            class="w-full resize-none rounded-lg border border-gray-200 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                        />
                        <div class="mt-2 flex gap-2">
                            <button
                                class="rounded-lg bg-green-600 px-4 py-1.5 text-xs font-semibold text-white hover:bg-green-700"
                                @click="review(e.id, 'approved')"
                            >
                                Mark reviewed
                            </button>
                            <button
                                class="rounded-lg bg-amber-500 px-4 py-1.5 text-xs font-semibold text-white hover:bg-amber-600"
                                @click="review(e.id, 'needs_revision')"
                            >
                                Request revision
                            </button>
                        </div>
        ...
        >
            <h2
                class="border-b border-gray-100 px-6 py-4 font-bold text-[#000928] dark:border-gray-700 dark:text-white"
            >
                Attendance
            </h2>
            <div
            ...
            <table v-else class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                    <tr v-for="a in attendance" :key="a.id">
                        <td
                            class="px-6 py-3 font-medium text-gray-700 dark:text-gray-200"
                        >
                            {{ a.date }}
                        </td>
                        ...
                        <td class="px-6 py-3">
                            <select
                                :value="a.status"
                                class="rounded-lg border border-gray-200 px-2 py-1 text-xs dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                                @change="
                                    markAttendance(
                                        a.date,
