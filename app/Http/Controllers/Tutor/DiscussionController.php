...
class DiscussionController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $query = Discussion::query()
            ->topLevel()
            ->whereHas('lesson.course', fn ($courseQuery) => $courseQuery->where('instructor_id', $user->id))
            ->with([
                'user:id,name',
        ...
        $discussions = $query
            ...
            ->through(fn (Discussion $discussion) => [
                ...
                'destination_url' => $discussion->lesson && $discussion->lesson->course
                    ? route('lms.courses.lessons.show', [
                        'course' => $discussion->lesson->course->id,
                        'lesson' => $discussion->lesson->id,
                    ])
                    : null,
            ]);

        return Inertia::render('Tutor/Discussions/Index', [
            'title' => 'Course Discussions',
            'subtitle' => 'Follow learner questions and answers across your courses.',
            'filters' => $filters,
